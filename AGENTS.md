...
**Offline sweep (Workers, ungraceful disconnects):** a hard power-off or network partition never fires `webSocketClose`/`webSocketError`, so the disconnect-first path above never runs and the DO happily reports "connected" until something else notices — Cloudflare will eventually reap the dead socket, but on no fixed schedule. `src/daemon/cell/offline-sweep.ts` closes that gap with a Cron Trigger (`wrangler.jsonc` `triggers.crons`, `"* * * * *"` — 60s, Cloudflare's finest cron granularity) that every tick: (1) reads servers Postgres currently believes are `connected` (`listConnectedServersForSweep`), (2) fans out a **read-only** `checkLiveness()` RPC (`DaemonCell.checkLiveness`, `do.ts` `/rpc/liveness`) to each one — it only reads the runtime-tracked WebSocket auto-response timestamp (`ctx.getWebSocketAutoResponseTimestamp`, the same value the free `DAEMON_CELL_PING`/pong keeps warm) and never touches SQLite, so a healthy server costs one Workers subrequest and nothing else, and (3) only for servers stale by more than `OFFLINE_SWEEP_STALE_MS` (90s) does it write to Postgres, reusing the existing `onDaemonDisconnected` projection, and call the `notifyServerWentOffline` hook (currently a structured log line — swap in real email/webhook delivery there once a notification target exists). Fan-out is capped at `MAX_SWEEP_FANOUT` (900, under the Workers-paid 1000-subrequest-per-invocation ceiling) and bounded to `FANOUT_CONCURRENCY` (25) in-flight RPCs at a time; beyond the cap, servers are simply picked up on a later tick. This does **not** reintroduce a per-DO recurring alarm — every `setAlarm()` reschedule is a billed SQLite row write (see "Alarm / hibernation" below and the DO billing model), so centralizing the "when to check" clock in one Cron Trigger keeps cost proportional to the number of *connected* servers rather than the number of ticks. Redis/Deno does not need this cron — it already has `sweepStalePresence` above. Detection latency: up to one cron tick plus the stale grace, so worst case is close to two minutes; typical case is under one.

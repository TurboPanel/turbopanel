...
**Host OS (`server.metadata.os`):** daemons send an `os` block on WS `hello` (from `/etc/os-release`). Deno WS and the Workers DO hello path call `touchServerMetadata` to merge it into `server.metadata.os` (idempotent). `resolveFleetPresence` surfaces `os` on the primary-DB enrichment path (not in the cached `servers-list` rows). `GET /api/client/v1/servers` returns `os` plus `osDisplay` from `formatServerOsDisplay()` (e.g. `"Debian 13 (Trixie)"`).

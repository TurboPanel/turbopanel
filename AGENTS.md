...
**Public sign-up:** `IS_SIGNUP_ENABLED` in the `setting` table is the panel-controlled toggle (default disabled when unset). `TURBOPANEL_IS_SIGNUP_ENABLED=1`/`true` or `0`/`false` is an optional **force** override that wins over the database. Live Wrangler **leaves `TURBOPANEL_IS_SIGNUP_ENABLED` unset** so the admin panel DB toggle controls sign-up without a deploy — never ship `"1"`/`"true"` on `env.live` (a config regression guard fails the build if live is force-enabled). Testing keeps `"1"` as a permanent force-enable. Local Tilt may still set `TURBOPANEL_IS_SIGNUP_ENABLED=1` in `.env.example` / `.dev.vars` as a force-enable for dev. A live force-disable (`"0"`) would also override the panel; prefer leaving the var unset.

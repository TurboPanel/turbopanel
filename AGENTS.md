...
**Public sign-up:** `IS_SIGNUP_ENABLED` in the `setting` table is the panel-controlled toggle (default disabled when unset). `TURBOPANEL_IS_SIGNUP_ENABLED=1`/`true` or `0`/`false` is an optional **force** override that wins over the database. Live Wrangler ships `"TURBOPANEL_IS_SIGNUP_ENABLED": "0"` so the binding always exists — dashboard-only adds of plaintext vars are wiped on the next `wrangler deploy`. To open production sign-up: set the live Worker var to `"1"` in the Cloudflare dashboard and **Deploy** that version. Never commit `"1"`/`"true"` on `env.live` (a config regression guard fails if live is force-enabled in git). While the live env force is `"0"`, the DB/panel toggle cannot open sign-up — the env force wins. Testing keeps `"1"` as a permanent force-enable. Local Tilt may still set `TURBOPANEL_IS_SIGNUP_ENABLED=1` in `.env.example` / `.dev.vars` as a force-enable for dev.

...
| `DAEMON_REST_RATE_LIMITER` | enroll / challenge / session / lease / decrypt / metrics | `"1002"` | `"2002"` | `"3002"` | `{ limit: 30, period: 60 }` |

Keys use stable identifiers (`serverId` / `licenseId`, or the `enroll-challenge` sentinel for anonymous enrollment challenges), never IP — see `src/daemon/rate-limit/keys.ts`. `DaemonRestRateLimitRoute` includes `"metrics"` gated via `enforceJwtRestLimit("metrics")` (key `daemon:rest:metrics:<serverId>`); metrics is ~1/min/server so the shared REST limiter cap is comfortable. Workers resolution lives in `resolveWorkersDaemonRateLimiters` (`workers-bindings.ts`); absent bindings become noops. After JWT verify on `GET /ws/daemon/v1`, a failed connect limit returns **429** before `DAEMON_CELL.getByName` so the Durable Object never wakes. REST returns `{ ok:false, error:"rate_limited" }` with **429**. Inside the DO, an in-memory per-`connectionId` inbound-message cap (`TURBOPANEL_DAEMON_WS_INBOUND_LIMIT` default 120 / `TURBOPANEL_DAEMON_WS_INBOUND_WINDOW_MS` default 60000) drops floods without timers (auto-response pings never hit `webSocketMessage`).
...
| `connected_at`, `last_seen_at`, `agent_json` | Postgres projection (`server.daemon.status` / `.projection`); admin/debug cell snapshot sources `connectedAt`/`lastInboundAt`/`lastSeenAt`/`agent`/`connected` from Postgres, `keyLastUsedAt`/`remoteAddress` from the `cell` row |
...
| `POST /api/daemon/v1/secrets/decrypt` | daemon JWT | Batch-decrypt recipient-scoped `tpdaemon.v1` envelopes (`{ ciphertexts }` → `{ plaintexts }`; null per failed entry); rejects global `tpsecret` blobs |

...
- **`instance`** (UID **9998**): runs the instance, Caddy, and the dev UI. Primary group is **`turbopaneli`** (GID **9998**); supplementary group **`turbopanel`** (GID 9999) grants socket and checkout access. **No sudo**. It **reads** platform checkouts via group membership; it must not own source files (that blocks reclaiming ownership to `turbopanel`).

...
import { type AuthRouteOpts, registerAuthnRoutes, registerAuthRoutes } from './authn/http.ts'
import {
...
export type ClientRouteOpts = AuthRouteOpts & {
  ...
  getOpenApiSpec?: (serverUrl: string, options?: ClientOpenApiOptions) => object
}
...
export function registerClientRoutes(app: Hono<AppEnv>, opts: ClientRouteOpts) {
  ...
  client.get('/status', async (c) => {
    ...
    try {
      ...
      return c.json({ ok: false, error: 'Database unavailable', code: 'database_error' }, 503)
    }
    if (payload === null) {

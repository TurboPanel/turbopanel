...
async function withProjectFixtures(
  ...
  try {
    ...
    for (const row of leftover) {
      ...
      await db.delete(managed).where(eq(managed.projectId, row.id))
...
test('POST /projects managed postgres scaffolds env without managed row', async () => {
  await withProjectFixtures(async ({
    ...
    assertEquals(metadata?.managed_id, undefined)
    ...
    const managedForProject = await db
      .select({ id: managed.id })
      .from(managed)
      .where(eq(managed.projectId, projectId))
    assertEquals(managedForProject.length, 0)

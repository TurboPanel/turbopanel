...
/** The org-wide project-name lock (`uniq_project_organization_name`) firing under a racer the pre-check missed. */
export function isProjectNameUniqueViolation(err: unknown): boolean {
  return isUniqueViolationOn(err, 'uniq_project_organization_name')
}

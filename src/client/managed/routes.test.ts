...
async function withManagedFixtures(
  ...
  if (!dbUrl) {
    console.warn(
      "Skipping managed route tests: TURBOPANEL_DATABASE_URL not set",
    );
    return;
  }
      ...
      if (leftoverProjectIds.length > 0) {
        ...
        if (leftoverEnvironmentIds.length > 0) {
          await db.delete(service).where(
            inArray(service.environmentId, leftoverEnvironmentIds),
          );
          await db.delete(managed).where(
            inArray(managed.environmentId, leftoverEnvironmentIds),
          );
          await db.delete(environment).where(
            inArray(environment.id, leftoverEnvironmentIds),
          );
        }
        await db.delete(project).where(inArray(project.id, leftoverProjectIds));
      }
      await db.delete(workspace).where(
        inArray(workspace.id, leftoverWorkspaceIds),
      );
    }
...
test("managed create returns rootPassword once, seals principal, is idempotent", async () => {
  await withManagedFixtures({}, async ({
    ...
    assertEquals(
      commandQueue.envelopes.every((envelope) =>
    ...
    const [managedRow] = await db
      ...
      .where(eq(managed.environmentId, environmentId))
      .limit(1);
    const options = managedRow?.options as {
      settings?: unknown;
      databases?: string[];
    };
...
test("PATCH rejects denylisted dockerOptions and does not enqueue", async () => {
  await withManagedFixtures({}, async ({
    ...
    const badBind = await app.request(
      `/environments/${environmentId}/managed`,
      {
        method: "PATCH",
        headers,
        body: JSON.stringify({
...
test("managed_busy guard on apply while status=applying", async () => {
  await withManagedFixtures({}, async ({
    ...
    const apply = await app.request(
      `/environments/${environmentId}/managed/apply`,
      {
        method: "POST",
        headers,
        body: "{}",
...
test("GET /environments/:id/managed/status returns status host port containers", async () => {
  await withManagedFixtures({}, async ({
    ...
    for (const row of body.containers) {
      ...
      assertEquals(
        row.role === "service" || row.role === "ingress" ||
          row.role === "turbopanel",
        true,
      );
    }
...
test("GET /environments/:id/managed/status returns last apply and ingress errors", async () => {
  await withManagedFixtures({}, async ({
    ...
    await db.update(managed).set({ status: "failed" }).where(
...
test("backup routes are forbidden without manage grant", async () => {
  await withManagedFixtures({ withManageGrant: false }, async ({
    ...
    const get = await app.request(
      `/environments/${environmentId}/managed/backups`,
      {
        headers,
      },
    );
...
test("GET backups returns stored metadata newest first, and delete/restore enqueue against a known record", async () => {
  await withManagedFixtures({}, async ({
    ...
    const removeBody = await remove.json() as {
      ok: boolean;
      commandId: string;
    };
...
test("backup create/delete/restore target managed.server_id when environment placement has drifted", async () => {
  await withManagedFixtures({}, async ({
    ...
    assertEquals(create.status, 200);
    const created = await create.json() as {
      managed: { id: string; serverId: string };
    };
    ...
    try {
      ...
      const backupCreate = await app.request(
        `/environments/${environmentId}/managed/backups`,
        {
          method: "POST",
          headers,
      ...
      const backupBody = await backupCreate.json() as {
        backupId: string;
        serverId: string;
      };
  ...
  await withManagedFixtures({}, async ({
    ...
    await withDriftedPlacement(
      ...
      async () => {
        ...
        assertEquals(
          newEnvelopes.some((envelope) =>
            envelope.type === "managed.ha.reconcile"
          ),
          true,
        );
      },
  ...
  await withManagedFixtures({}, async ({
    ...
    await withDriftedPlacement(
      ...
      async () => {
        ...
        const destroyBody = await destroy.json() as {
          deleted: boolean;
          serverId: string;
        };
...
test("POST root-password includes redeployRequired when bindings exist; DELETE user/db block on bindings", async () => {
  await withManagedFixtures({}, async ({
    ...
    try {
      ...
      console.warn(
        "Skipping binding impact managed tests: binding table not applied",
      );
      return;
    }
    ...
    assertEquals(
      rotateBody.redeployRequired.services[0]?.serviceId,
      consumerServiceId,
    );
    assertEquals(
      rotateBody.redeployRequired.services[0]?.keyPrefix,
      "DATABASE",
    );
    assertEquals(
      rotateBody.redeployRequired.services[0]?.environmentId,
      environmentId,
    );
    ...
    assertEquals(
      deleteUserBody.services.some((s) => s.serviceId === consumerServiceId),
      true,
    );
  ...
  await withManagedFixtures({}, async ({
    ...
    await withDriftedPlacement(
      ...
      async () => {
        ...
        assertEquals(
          afterCreateEnvelopes.every((envelope) =>
            envelope.serverId === serverId
          ),
          true,
        );
        ...
        const afterDeleteEnvelopes = commandQueue.envelopes.slice(
          beforeCount + 3,
        );
        assertEquals(afterDeleteEnvelopes.length, 3);
        assertEquals(
          afterDeleteEnvelopes.every((envelope) =>
            envelope.serverId === serverId
          ),
          true,
        );
      ...
      async () => {
        ...
        const createDb = await app.request(
          `/environments/${environmentId}/managed/databases`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({ name: "drift_db" }),
          },
        );
...
test("POST user rejects invalid username and DELETE database protects initial database", async () => {
  await withManagedFixtures({}, async ({
    ...
    const dropInitial = await app.request(
      `/environments/${environmentId}/managed/databases/${
        encodeURIComponent("defaultdb")
      }`,
      { method: "DELETE", headers },
    );
    assertEquals(dropInitial.status, 409);
    assertEquals(await dropInitial.json(), {
...
test("registerManagedRoutes requires session secrets", () => {
  ...
  try {
    ...
    assertEquals(
      ...
      "session secrets are required for managed routes",
    );
  }

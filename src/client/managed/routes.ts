...
type ManagedRow = NonNullable<
  Awaited<ReturnType<typeof findManagedForEnvironment>>
>;
...
async function runApplyForManaged(
  ...
  const enqueued = await enqueuePreparedManagedApply(
    c,
    db,
    prepared.commandQueue,
    {
      userId,
...
> {
  ...
  if (params.replicaClass === "failover") {
    const transportErr = assertFailoverReplicaTransportAllowed(
      toPrimary.transport,
    );
    if (transportErr) {
  ...
  if (
    replicaPlacementNeedsDatacenter(toPrimary.transport, params.replicaClass)
  ) {
...
async function resolveFailoverConversionPlacement(
  ...
  if (replicaPlacementNeedsDatacenter(toPrimary.transport, "failover")) {
    const dcReady = await assertServerDatacenterReady(
      db,
      params.memberServerId,
    );
    if (dcReady) {
      return c.json({ error: dcReady.kind }, 422);
    }
...
async function applyMemberReplicaClassPatch(
  ...
  const primaryServerId = resolveManagedTargetServerId(
    c,
    params.managedServerId,
  );
  ...
  const conversion = evaluateReplicaClassConversion(
    member,
    replicaClass,
    placementOk,
  );
  ...
  const converted = await updateManagedMemberReplicaClass(
    db,
    member.id,
    replicaClass,
  );
...
export function registerManagedRoutes(
  ...
  router.post("/environments/:id/managed", async (c) => {
    ...
    const ctx = await loadManagedContext(
      c,
      db,
      environmentId,
      auth.organizationId,
  ...
  router.post("/environments/:id/managed/root-password", async (c) => {
    ...
    const redeployRequired = await listBindingImpactForPrincipal(
      db,
      rootPrincipalId,
    );
    return c.json({
  ...
  router.post(
    ...
    async (c) => {
      ...
      const auth = await authorizeManagedRequest(
        c,
        db,
        environmentId,
        "manage",
      ...
      const [target] = await db
        ...
        .from(principal)
        .where(
          and(eq(principal.id, principalId), eq(principal.managedId, row.id)),
        )
        .limit(1);
      ...
      if (!("ok" in materializeResult)) {
        await restorePreviousPrincipalPassword(
          db,
          principalId,
          previousPassword,
        );
  ...
  router.delete(
    ...
    async (c) => {
      ...
      if (
        await hasBindingsForDatabase(db, { managedId: row.id, databaseName })
      ) {
  ...
  router.post("/environments/:id/managed/members", async (c) => {
    ...
    if (!replicaClassParsed.ok) {
      return c.json(
        { error: replicaClassParsed.error },
        replicaClassParsed.status,
      );
    }
  ...
  router.post("/environments/:id/managed/backups", async (c) => {
    ...
    const database = resolveBackupDatabase(
      options,
      body.database,
      ctx.spec.engine,
    );
  ...
  router.post(
    ...
    async (c) => {
      ...
      if (isManagedBackupApiError(built)) {
        return mapManagedBackupApiError(
          c,
          built,
        );
      }
  ...
  router.get("/organizations/:id/managed", async (c) => {
    ...
    const serverNames = serverIds.length === 0 ? [] : await db

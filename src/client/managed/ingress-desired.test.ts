...
async function withSingleClusterIngressFixture(
  ...
  fn: (ctx: {
    ...
    dataEncryptionSecrets: Awaited<ReturnType<typeof testEncryptionContext>>['dataEncryptionSecrets']
  }) => Promise<void>,
): Promise<void> {
  if (!dbUrl) {
    console.warn('Skipping ingress-desired bind-address tests: TURBOPANEL_DATABASE_URL not set')
    return
  }
  ...
  await attachDaemonStateToServer(db, serverId, {
    publicJwk: { kty: 'OKP', crv: 'Ed25519', x: `ingress-desired-key-${serverId}` },
  ...
  try {
    await fn({ db, serverId, organizationId, secretsConfig, dataEncryptionSecrets })
  } finally {
...
test('unionExposureBind picks the most permissive scope', () => {
  ...
  assertEquals(unionExposureBind(['datacenter', undefined, 'local']), 'datacenter')
})

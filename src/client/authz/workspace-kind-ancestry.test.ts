...
test("resolveWorkspaceKindForEntity returns kind for managed-scoped principals", async () => {
  await withAncestryFixtures(
    async ({ db, organizationId, userWorkspaceId, systemWorkspaceId }) => {
      const [userProject] = await db
        .insert(project)
        .values({ name: "User Managed Project", workspaceId: userWorkspaceId, organizationId })
      ...
      const [userManaged] = await db
        .insert(managed)
        .values({ environmentId: userEnvironmentId, engine: "postgres" })
      ...
      const [systemManaged] = await db
        .insert(managed)
        .values({ environmentId: systemEnvironmentId, engine: "postgres" })
        .returning({ id: managed.id });
      ...
      try {
        ...
        await db.delete(environment).where(
          eq(environment.id, userEnvironmentId),
        );
        await db.delete(environment).where(
          eq(environment.id, systemEnvironmentId),
        );
...
test("resolveWorkspaceKindForEntity walks workspace, project, and environment ancestry", async () => {
  await withAncestryFixtures(
    async ({ db, organizationId, userWorkspaceId, systemWorkspaceId }) => {
      const [userProject] = await db
        .insert(project)
        .values({ name: "User Project", workspaceId: userWorkspaceId, organizationId })
        .returning({ id: project.id });
      const userProjectId = userProject!.id;
      ...
      const [systemProject] = await db
        .insert(project)
        .values({ name: "System Project", workspaceId: systemWorkspaceId, organizationId })
        .returning({ id: project.id });
      const systemProjectId = systemProject!.id;
...
test("resolveWorkspaceKindForEntity resolves service and hosting descendants", async () => {
  await withAncestryFixtures(async ({ db, organizationId, systemWorkspaceId }) => {
    const [systemProject] = await db
      .insert(project)
      .values({
        name: "System Service Project",
        workspaceId: systemWorkspaceId,
        organizationId,
      })
      .returning({ id: project.id });
...
test("resolveWorkspaceKindForEntity resolves environment-scoped variables", async () => {
  await withAncestryFixtures(async ({ db, organizationId, userWorkspaceId }) => {
    const [userProject] = await db
      .insert(project)
      .values({ name: "Variable Project", workspaceId: userWorkspaceId, organizationId })
      .returning({ id: project.id });
    const userProjectId = userProject!.id;

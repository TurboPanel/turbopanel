...
import { organization, grant, user, session as sessionTable } from '../../lib/db/schema.ts'
...
describe('compose-privileged-fields real-Postgres end-to-end', () => {
  it('PUT persists to a real org row through real session auth, and the real row then drives a real deploy-time decision', async () => {
    if (!env.HYPERDRIVE) {
      console.warn('skipping real-Postgres compose-privileged-fields test: no HYPERDRIVE binding')
      return
    }
    ...
    try {
      ...
      expect(await getBefore.json()).toEqual({ composeGatedFieldsEnabled: false })

      const put = await app.request(`/organizations/${org!.id}/compose-privileged-fields`, {
        method: 'PUT',
        headers: { Cookie: cookie, 'Content-Type': 'application/json' },
        body: JSON.stringify({ composeGatedFieldsEnabled: true }),
      })
      expect(put.status).toBe(200)
      expect(await put.json()).toEqual({ ok: true, composeGatedFieldsEnabled: true })
      ...
      const privilegedDoc: ComposeDocument = {
        version: 1,
        data: { services: { web: { image: 'nginx:alpine', privileged: true } } },
      ...
      const putOff = await app.request(`/organizations/${org!.id}/compose-privileged-fields`, {
        method: 'PUT',
        headers: { Cookie: cookie, 'Content-Type': 'application/json' },
      ...
      expect(
        validateComposeForDeploy(privilegedDoc, {
          composeGatedFieldsEnabled: resolveComposeGatedFieldsEnabled(optionsAfter),
        })?.kind

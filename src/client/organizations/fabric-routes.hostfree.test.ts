...
test("parseRelayPatchBody rejects non-objects and oversized advertisedCidrs", () => {
  assertEquals(parseRelayPatchBody(null), {
    ok: false,
    error: "Invalid request",
  });
  assertEquals(parseRelayPatchBody([]), {
    ok: false,
    error: "Invalid request",
  });
  assertEquals(
...
test("parseFabricPutBody accepts an IPv4 containerPool wide enough for a relay /16", () => {
  assertEquals(
    parseFabricPutBody({ enabled: true, containerPool: " 10.64.0.0/10 " }),
    {
      ok: true,
      enabled: true,
  ...
  assertEquals(
    parseFabricPutBody({ enabled: true, containerPool: "10.64.0.0/16" }),
    {
      ok: true,
      enabled: true,
...
test("bindSecretEncryptFn returns null without secrets and binds encrypt otherwise", async () => {
  assertEquals(
    bindSecretEncryptFn(
      undefined,
      (secrets, plaintext) => Promise.resolve(`${secrets}:${plaintext}`),
    ),
    null,
  );
  const encrypt = bindSecretEncryptFn(
    "key",
    (secrets, plaintext) => Promise.resolve(`${secrets}:${plaintext}`),
  );
...
test("fabricEnableErrorResponse maps CIDR and pool exhaustion to 409", async () => {
  const cidr = fabricEnableErrorResponse(
    new Error("No free CIDR left in pool"),
  );

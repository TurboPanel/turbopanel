...
test('hosting CRUD covers list, filter, create, patch, and delete', async () => {
  await withHostingFixtures(async ({
    ...
    const listBody = await list.json() as { hostings: Array<{ id: string; displayName: string | null }> }
    ...
    const filteredBody = await filtered.json() as { hostings: Array<{ id: string }> }
    ...
    const detailBody = await detail.json() as {
      hosting: { id: string; displayName: string | null; description: string | null }
    }
...
test('POST /hostings accepts tcp protocol with port mappings', async () => {
  await withHostingFixtures(async ({
    ...
    const body = await detail.json() as {
      hosting: { options: { protocol?: string; ports?: Array<{ published: number; target: number }> } }
    }
...
test('POST /hostings returns 404 when service belongs to another org', async () => {
  await withHostingFixtures(async ({
    ...
    const [foreignWs] = await db
      .insert(workspace)
      .values({ organizationId: foreignOrg!.id, name: 'FW', createdAt: now, updatedAt: now })
      .returning({ id: workspace.id })
    const [foreignProj] = await db
      .insert(project)
      .values({ workspaceId: foreignWs!.id, name: 'FP', createdAt: now, updatedAt: now })
      .returning({ id: project.id })
    const [foreignEnv] = await db
      .insert(environment)
      .values({ projectId: foreignProj!.id, name: 'FE', createdAt: now, updatedAt: now })
      .returning({ id: environment.id })

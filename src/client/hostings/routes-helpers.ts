 ...
 * `reconcile-hostings.ts`'s deploy-prepare stamp may set these. Stripped from
 * every client-supplied `metadata`, matching the same convention
 * `CONTAINER_PROMOTED_METADATA_KEYS` / `SERVICE_PROMOTED_METADATA_KEYS` /
 * `ENVIRONMENT_PROMOTED_METADATA_KEYS` already use (schema-sql-keys,
 * Road-to-0.1.x): before this, a client could set `composeOwned: true` on
 * their own row through the open `metadata` passthrough and trip
 * `assertHostingNotComposeOwnedOr409`'s guard on a row compose never
 * actually declared.
 */
export const HOSTING_PROMOTED_METADATA_KEYS = [
  HOSTING_COMPOSE_OWNED_METADATA_KEY,
  HOSTING_COMPOSE_SERVICE_METADATA_KEY,
  HOSTING_COMPOSE_ROUTE_METADATA_KEY,
  HOSTING_COMPOSE_TLS_MODE_METADATA_KEY,

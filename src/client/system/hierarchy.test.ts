...
test("ensureSystemHierarchy is idempotent for the same org/server", async () => {
  await withHierarchyFixtures(async ({ db, organizationId, serverId }) => {
    ...
    const [ingressProjectRow] = await db
      .select({ component: project.component, metadata: project.metadata })
      .from(project)
      .where(eq(project.id, first.projectId))
      .limit(1);
    assertEquals(
      ingressProjectRow?.component,
      SYSTEM_HOSTING_INGRESS_COMPONENT,
    );
    assertEquals(
...
test("hierarchy ensure reuses an install-created System workspace", async () => {
  await withHierarchyFixtures(async ({ db, organizationId, serverId }) => {
    ...
    const ingress = await ensureSystemHierarchy(db, {
      organizationId,
      serverId,
    });
...
test("ensureSelfHostSystemHierarchy is idempotent for the same org/server", async () => {
  await withHierarchyFixtures(async ({ db, organizationId, serverId }) => {
    const first = await ensureSelfHostSystemHierarchy(db, {
      organizationId,
      serverId,
    });
    const second = await ensureSelfHostSystemHierarchy(db, {
      organizationId,
      serverId,
    });
...
test("ensureSelfHostSystemHierarchy provisions database/queue with pending uuid-named containers", async () => {
  await withHierarchyFixtures(async ({ db, organizationId, serverId }) => {
    const result = await ensureSelfHostSystemHierarchy(db, {
      organizationId,
      serverId,
    });
    ...
    assertEquals(
      ...
      SYSTEM_SELF_HOST_COMPOSE_SERVICE_NAMES.length,
    );
    assertEquals(
    ...
    const [projectRow] = await db
      .select({ metadata: project.metadata, component: project.component })
      .from(project)
...
test("findSystemEnvironmentForServer filters by project component", async () => {
  await withHierarchyFixtures(async ({ db, organizationId, serverId }) => {
    ...
    assertEquals(
      await findSystemEnvironmentForServer(
        db,
        serverId,
        SYSTEM_HOSTING_INGRESS_COMPONENT,
      ),
    ...
    assertEquals(
      firstMatch === ingress.environmentId ||
        firstMatch === selfHost.environmentId,
      true,
    );
  });
});

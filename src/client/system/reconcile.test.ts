    ...
    if (projectIds.length > 0) {
      ...
      if (environmentIds.length > 0) {
        ...
        if (serviceIds.length > 0) {
          ...
          await db.delete(hosting).where(
            inArray(hosting.serviceId, serviceIds),
          );
...
async function withReconcileFixtures(
  ...
  const [insertedServer] = await db
    ...
    .values({
      ...
      isHostingEnabled: hostingEnabled,
      createdAt: now,
      updatedAt: now,
    })
...
test("buildSystemReconcilePayload returns one payload per system environment", async () => {
  await withReconcileFixtures(
    { selfHost: true, httpIngressDemand: true },
    async ({ db, serverId }) => {
      ...
      assertEquals(
        hostingIngress?.environmentId === selfHost?.environmentId,
        false,
      );
...
test("enqueueSystemReconcileIfConnected skips when the daemon is offline", async () => {
  await withReconcileFixtures(
    ...
    async ({ db, serverId, queue }) => {
      const result = await enqueueSystemReconcileIfConnected(
        db,
        queue,
        serverId,
      );

...
test("isIpAddressUniqueViolation matches the org address index", () => {
  ...
  const otherErr = Object.assign(
    new Error(
      'duplicate key value violates unique constraint "uniq_ip_vpn_address"',
    ),
    { code: "23505" },
  );
  ...
  assertEquals(
    isIpAddressUniqueViolation({ code: "23505", message: "other" }),
    false,
  );
});
...
test("parseCreateIpAddress rejects client-supplied version and invalid addresses", async () => {
  ...
  await expectInvalidRequest(
    parseCreateIpAddress(c, { address: "203.0.113.10", version: 4 }),
  );
...
test("assertIpScopeFkRules enforces free-pool and membership pin rules", async () => {
  ...
  // Free pool (datacenter only) is OK. Membership pin requires networkId.
  assertEquals(
    assertIpScopeFkRules(c, "datacenter", { datacenterId: "dc-1" }),
    null,
  );
  await expectInvalidRequest(
...
test("assertDatacenterMembershipNetwork requires a same-datacenter site subnet", async () => {
  const c = mockContext();

  await expectInvalidRequest(
    assertDatacenterMembershipNetwork(c, "10.0.0.10", "dc-1", null),
  );
  await expectInvalidRequest(
    assertDatacenterMembershipNetwork(c, "10.0.0.10", "dc-1", {
      kind: "docker",
      datacenterId: "dc-1",
      cidr: "10.0.0.0/24",
    }),
  ...
  await expectInvalidRequest(
    assertDatacenterMembershipNetwork(c, "10.0.0.10", "dc-1", {
      kind: "datacenter",
      datacenterId: "dc-other",
      cidr: "10.0.0.0/24",
    }),
  );

  const outside = assertDatacenterMembershipNetwork(c, "10.1.0.10", "dc-1", {
    kind: "datacenter",
    datacenterId: "dc-1",
    cidr: "10.0.0.0/24",
  });
  if (!(outside instanceof Response)) throw new TypeError("expected response");
  assertEquals(outside.status, 400);
  assertEquals(await outside.json(), { error: "address_not_in_any_subnet" });
...
test("parseCreateIpEnums validates allocation and scope", async () => {
  ...
  const badAllocation = parseCreateIpEnums(c, {
    allocation: "pool",
    scope: "public",
  });
  if (!(badAllocation instanceof Response)) {
    throw new TypeError("expected response");
  }
  assertEquals(badAllocation.status, 400);
  const badScope = parseCreateIpEnums(c, {
    allocation: "dedicated",
    scope: "vpn",
  });
...
test("parseEnumQueryFilter and parseScopeFkUuid validate query and body UUIDs", async () => {
  ...
  assertEquals(
    parseEnumQueryFilter(c, "scope", new Set(["public", "datacenter"])),
    "public",
  );
  const badScope = parseEnumQueryFilter(
    mockContext({ scope: "loopback" }),
    "scope",
    new Set(["public"]),
  );
  if (!(badScope instanceof Response)) throw new TypeError("expected response");

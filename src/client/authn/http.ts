...
import { compatLogError, compatLogInfo, compatLogWarn } from '../../lib/log-compat.ts'
...
import { parseTrustedProxyCidrs, resolvePeerAddress } from '../../lib/peer-address.ts'
...
function readSessionCookie(c: Context, runtime: 'deno' | 'workers'): string | null {
 ...
 */
export function resolveClientIp(c: Context, runtime: 'deno' | 'workers'): string | null {
  return (
    resolvePeerAddress(
      {
      ...
      { runtime, trustedProxyCidrs: trustedProxyCidrs() }
    )?.address ?? null
  )
}
 ...
 */
function resolveAuthRateLimiter(c: Context, runtime: 'deno' | 'workers'): AuthRateLimiter {
...
export type AuthBodyValidation<T> = { ok: true; value: T } | { ok: false; error: string }
...
type ParsedSignInBody = { ok: true; email: string; password: string } | { ok: false; error: string }
...
async function resolveSignupGate(
  ...
  if (
    ...
    const invited = invitationGate?.invitationId
      ? await invitationAllowsSignup(db, invitationGate.invitationId, invitationGate.email)
      : false
    if (!invited) {
      return {
        ok: false,
  ...
  const env =
    platformEnv ??
    (opts.runtime === 'deno' && typeof Deno !== 'undefined' ? Deno.env.toObject() : {})
  const dataEncryptionSecrets = c.get('dataEncryptionSecrets')
  const emailSettings = await resolveEmailSettings(db, env, dataEncryptionSecrets)
  const emailVerificationEnabled = isEmailActiveForRuntime(emailSettings, opts.runtime)
  ...
  const emailQueue =
    getEmailQueue(c) ??
    (opts.runtime === 'workers' ? emailQueueFromResolvedSettings(emailSettings) : undefined)
  const emailFrom =
    emailSettings.from || c.get('emailFrom') || opts.emailFrom || 'noreply@turbopanel.local'
...
export function registerAuthRoutes(app: Hono<AppEnv>, opts: AuthRouteOpts) {
  ...
  auth.post('/sign-out', async (c) => {
    ...
    const clearPrimary = `${tls.cookieName}=; ${clearAttrs}${tls.isHttps ? '; Secure' : ''}`

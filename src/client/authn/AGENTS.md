...
- `DAEMON_INBOUND_ALLOWED` in `src/contracts/cell-protocol.ts` is a static set of accepted post-auth message types — not an authz system.
...
When the developer surface is enabled (`TURBOPANEL_DEV_SURFACE=1`, written only by the daemon's `turbopanel-instance.service.j2` for co-located Deno source-mode dev), `createDeveloperAccessMiddleware` accepts either a superadmin session cookie **or** an HMAC `Authorization: Local-Console …` credential from the Ink console over the Unix socket (`src/developer/local-console-auth.ts`; client: `~/dev/src/lib/developer-client.ts`).
...
**Delivery:** at deploy/delivery time the instance decrypts the at-rest `tpsecret` envelope and re-seals it as a recipient-bound `tpdaemon.v<version>.<serverId>.<keyId>.<payloadB64u>` envelope via the shared `resealSecretForDaemon` helper (`src/lib/secrets/data-encryption.ts`). Daemons decrypt only those recipient-scoped envelopes through `POST /api/daemon/v1/secrets/decrypt` (daemon JWT). Global `tpsecret` blobs are never handed to daemons. The co-located daemon's instance secrets follow the same rule when it advertises `sealed-instance-secrets-v1`: an uploaded control-plane key travels on `public-urls-update` as `keyEnvelope` (resealed straight from the at-rest `tpsecret`) and a non-empty Cloudflare tunnel token on `tunnel-token` as `tokenEnvelope`, so neither sits in plaintext in the cell outbox (Redis stream or Durable Object storage). `src/features/install/instance-secret-sealing.ts` decides; a daemon without the feature still gets the legacy `keyPem` / `token` fields until `MIN_SUPPORTED_DAEMON_VERSION` passes the release that ships it, when those fields are removed.

**Git provider App credentials:** registered GitHub Apps and GitLab OAuth applications live in the **`gitapp`** table (`src/features/git/forge-records.ts`), one row per application — an instance may hold several, scoped instance-wide (`organization_id IS NULL`) or to one organization. Each row's `credentials` jsonb holds the App private key, the OAuth client secret, and the webhook secret, each sealed as `tpsecret` exactly like TLS private keys — never plaintext at rest, and never returned by either API surface (`/api/admin/v1/forges` and `/api/client/v1/forges` report presence only). **Installation access tokens are minted on demand and never persisted**: `src/features/git/github-app-token.ts` signs a ≤10-minute RS256 App JWT with Web Crypto (`RSASSA-PKCS1-v1_5` / SHA-256; PKCS#1 PEMs are wrapped into PKCS#8 locally, no Node `Buffer`), exchanges it for an installation token, hands the token to the caller in memory, and writes it nowhere — resolving the signing key from the app named by `installation.app_id`, not from a singleton. The install redirect carries a signed `state` (`tpinstall.v<version>.<payloadB64u>.<sigB64u>`, HKDF purpose `github-app-install-state`) naming both the organization *and* the application, so neither can be forged; the manifest flow uses the same envelope under a distinct purpose (`github-app-manifest-state`). The GitLab webhook token is additionally indexed by a domain-separated SHA-256 digest so an unscoped delivery can be routed in one lookup — the digest finds the row, the sealed value still authenticates it.
...
- **`certificate.key_pem`** (uploaded control-plane certificates, stage `certificates`): re-seal older-version `tpsecret` only (`allowDaemonBound: false`); plaintext / `tpdaemon` material is **failed**.
...
- **Bound to the starting browser + PKCE (one secret):** `/start` mints an RFC 7636 verifier, keeps it in an HttpOnly flow cookie (`__Host-turbopanel.oauth_flow` on HTTPS, `turbopanel.oauth_flow` otherwise; `SameSite=Lax`, 10-minute `Max-Age` = state TTL), signs its `S256` challenge into the state as `nonce`, and sends the challenge to the provider. The callback recomputes the challenge from the cookie and requires it to equal the signed nonce — a signed state alone only proves the instance minted it, and an attacker can start a flow and hand the victim the callback link (login CSRF) — then sends the verifier at the token exchange. The cookie is cleared on every callback response, so session cookies are **appended** (`c.header(…, { append: true })`), never set over it. Comparing a hash of a secret to a public signed value needs no constant-time helper.
...
| `GET` | `/api/client/v1/auth/oauth/:provider/start` | Public; redirect to GitHub or Google authorize URL with an `S256` PKCE challenge, setting the HttpOnly OAuth flow cookie. Unconfigured provider → **404**. `link=1` requires a session. Rate-limited `oauth-start` |
| `GET` | `/api/client/v1/auth/oauth/:provider/callback` | Public; provider redirect. Redirect-only (never JSON): session cookie, `/sign-in?challenge=` for 2FA, `/account/security?linked=` when linking, or `/sign-in?error=` (`oauth_state_invalid` — also when the flow cookie is missing or does not match the state, `oauth_exchange_failed`, `account_disabled`, `oauth_signup_disabled`, `oauth_email_unverified`, `account_conflict`, `not_configured`, `database_unavailable`). Clears the flow cookie on every response. Link-mode callback re-reads the live session and requires `session.userId === claims.linkUserId`; missing or mismatched session redirects to `/account/security?linked=&error=oauth_unauthenticated`. Other link-mode failures redirect to `/account/security?linked=&error=`. Persists identity only — never provider tokens. Rate-limited `oauth-callback` |
...
| `GET` | `/api/client/v1/workspaces` | List workspaces visible via `listVisible` (org-level `organization:own` / `organization:manage` grants); full CRUD table in `src/db/AGENTS.md` |
...
| File | Purpose |
| --- | --- |
| `src/lib/secrets/envelope.ts` | Shared envelope grammar (`formatEnvelope` / `parseEnvelope` / `hasEnvelopeScheme`); Workers-bundle-safe |
...
| `src/lib/secrets/password.ts` | Argon2id hash/verify for credential accounts |

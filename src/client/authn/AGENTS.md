...
- Cookie name: `turbopanel.session_token` on HTTP, `__Host-turbopanel.session_token` on HTTPS (resolved from the request URL in `src/client/authn/crypto.ts`). `__Host-` requires `Secure`, `Path=/`, and no `Domain` (stronger against subdomain cookie shadowing than the retired `__Secure-` name). Sign-out clears both the current HTTPS name and the legacy `__Secure-turbopanel.session_token` during the pre-MVP transition.

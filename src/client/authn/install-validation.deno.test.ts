...
it('validateOrganizationName enforces length and character set', () => {
  assertEquals(validateOrganizationName('Acme Corp'), null)
  assertEquals(validateOrganizationName(''), 'Organization name must be 1–255 characters')
  assertEquals(
    validateOrganizationName('x'.repeat(256)),
    'Organization name must be 1–255 characters',
  )
  assertEquals(
    validateOrganizationName('Bad/name'),
    'Organization name may only contain letters, numbers, spaces, and . _ -',
  )
})
...
it('validateSuperadminPassword mirrors install password policy', () => {
  assertEquals(validateSuperadminPassword(' passw0rd! '), 'Password must not have leading or trailing whitespace')
  assertEquals(validateSuperadminPassword('short1!'), 'Password must be at least 8 characters')
  assertEquals(validateSuperadminPassword('abcdefgh!'), 'Password must include at least one number')
  assertEquals(

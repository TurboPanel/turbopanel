...
it("spawnEnvWithoutLoaderVars drops the dynamic-loader variables and keeps the rest", () => {
  const prev = Deno.env.get("LD_LIBRARY_PATH");
  Deno.env.set("LD_LIBRARY_PATH", "/opt/turbopanel/vendor/duckdb/lib");
  try {
    const env = spawnEnvWithoutLoaderVars({ TP_PAM_USERNAME: "alice" });
    assertEquals(env.LD_LIBRARY_PATH, undefined);
    assertEquals(env.LD_PRELOAD, undefined);
    assertEquals(env.TP_PAM_USERNAME, "alice");
    assertEquals(env.PATH, Deno.env.get("PATH"));
  } finally {
    if (prev === undefined) Deno.env.delete("LD_LIBRARY_PATH");
    else Deno.env.set("LD_LIBRARY_PATH", prev);
  }
});

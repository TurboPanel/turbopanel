      ...
      if (environmentIds.length > 0) {
        ...
        if (serviceIds.length > 0) {
          await db.delete(variable).where(
            inArray(variable.serviceId, serviceIds),
          );
          await db.delete(binding).where(
            inArray(binding.serviceId, serviceIds),
          );
...
async function withBindingFixtures(
  ...
  const { app, secrets, dataEncryptionSecrets } =
    await createBindingRoutesTestApp(
      db,
      secretsConfig,
    );
  if (!dataEncryptionSecrets) {
    throw new TypeError(
      "expected data encryption secrets for binding fixtures",
    );
  }
  ...
  const sealedPassword = await encryptSecret(
...
test("POST /bindings rejects non-bindable principals and encryption gaps", async () => {
  await withBindingFixtures(async ({
    ...
    assertEquals(await noPwCreate.json(), {
      error: "binding_password_unavailable",
    });

    const { app: noEncApp, secrets: noEncSecrets } =
      await createBindingRoutesTestApp(
        db,
        secretsConfig,

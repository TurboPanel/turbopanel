...
function fakeDb(config: FakeDbConfig = {}): Db {
  ...
  return {
    ...
    delete: () => ({
      where: () => ({
        returning: () =>
          Promise.resolve(config.deleteEmpty ? [] : [{ id: APP_ID }]),
      }),
    }),
...
test("list serializes visible apps against the first usable public origin", async () => {
  ...
  assertEquals(
    first.webhookUrl,
    "https://panel.example.com:8443/webhook/github",
  );
});
...
test("get rejects a non-uuid, a hidden row, and a vanished summary", async () => {
  const badId = await invoke((c) =>
    getForgeHandler(c, fakeDb(), ORG, "not-a-uuid")
  );
...
test("create and patch refuse a forge address this instance must never dial", async () => {
  const cases: Array<[Record<string, string>, string, string]> = [
    ...
    [
      { baseUrl: "https://user:pw@gitlab.example.com" },
      "baseUrl",
      "credentials_in_url",
    ],
  ];
  for (const [urls, field, reason] of cases) {
    ...
    assertEquals(await jsonOf(created), {
      error: "forge_url_rejected",
      field,
      reason,
    });
  }
...
test("create maps conflict, validation, and unexpected write failures", async () => {
  ...
  const conflict = await invoke(
    (c) =>
      createForgeHandler(c, fakeDb({ insertError: uniqueViolation() }), ORG),
    { method: "POST", body },
  );
...
test("patch refuses a bad id, missing encryption, a hidden row, and a foreign write", async () => {
  const badId = await invoke(
    (c) => patchForgeHandler(c, fakeDb(), ORG, "nope"),
    {
      method: "PATCH",
      body: { name: "Renamed" },
    },
...
test("patch maps invalid body, missing row, conflict, and ForgeError", async () => {
  ...
  const conflict = await invoke(
    (c) =>
      patchForgeHandler(
        c,
        fakeDb({ appRows: [appRow()], updateError: uniqueViolation() }),
        ORG,
        APP_ID,
      ),
    { method: "PATCH", body: { name: "Renamed" } },
...
test("delete is 404 unless the caller can see and write the row, then 204", async () => {
  const badId = await invoke((c) =>
    deleteForgeHandler(c, fakeDb(), ORG, "nope")
  );
  assertEquals(badId.status, 404);
...
test("sync refuses a bad id, missing encryption, a hidden or read-only row, and gitlab", async () => {
  const badId = await invoke(
    (c) => syncForgeHandler(c, fakeDb(), ORG, "nope"),
    {
      method: "POST",
    },
  );
...
test("sync writes GitHub metadata and keeps isPublic when GitHub omitted it", async () => {
  ...
  const db = fakeDb({
    publicUrls: ["https://panel.example.com"],
    appRows: [
      appRow({ envelopes: { privateKeyEnvelope: sealed }, isPublic: true }),
    ],
  });
...
test("start manifest requires signing, a published origin, and a valid wizard body", async () => {
  ...
  assertEquals(
    (await jsonOf(unknownOrigin)).error,
    "webhook_origin_not_published",
  );
});
...
test("start manifest pins the org in callback URLs and keeps the app private", async () => {
  ...
  assertEquals(
    fields.redirect_url,
    `https://panel.example.com:8443/api/client/v1/forges/github/manifest/callback?organizationId=${ORG_ID}`,
  );
  assertEquals(
    (fields.callback_urls as string[])[0],
    `https://panel.example.com:8443/api/client/v1/repositories/github/callback`,
  );
  ...
  assertEquals(
    (hooks as Record<string, unknown>).url,
    "https://hooks.example.com:8443/webhook/github",
  );
  assertEquals(
    body.createUrl,
...
test("start manifest for the admin surface is public and uses the admin callback", async () => {
  ...
  assertEquals(
    ...
    "https://panel.example.com:8443/api/admin/v1/forges/github/manifest/callback",
  );
  assertEquals(
...
test("complete manifest redirects when signing, encryption, or the query is missing", async () => {
  ...
  assertEquals(
    noEncrypt.headers.get("location"),
    `/${ORG_ID}/projects/git-sources?error=unavailable`,
  );

  const missing = await invoke((c) =>
    completeGithubManifestHandler(c, fakeDb(), ORG)
  );
  assertEquals(
    missing.headers.get("location"),
    `/${ORG_ID}/projects/git-sources?error=invalid_request`,
  );
  ...
  assertEquals(
    ...
    `/${ORG_ID}/projects/git-sources?error=state_invalid`,
  );
});
...
test("complete manifest rejects a state minted for another organization", async () => {
  ...
  const response = await invoke(
    (c) => completeGithubManifestHandler(c, fakeDb(), ORG),
    {
      path: `http://tp.test/callback?state=${
  ...
  assertEquals(
    ...
    `/${ORG_ID}/projects/git-sources?error=forbidden`,
  );
});
...
test("complete manifest maps conversion failure, conflict, create failure, and success", async () => {
  ...
  const path = `http://tp.test/callback?state=${
    encodeURIComponent(state)
  }&code=one-shot`;
    ...
    async () => {
      const conflict = await invoke(
        (c) =>
          completeGithubManifestHandler(
            c,
            fakeDb({ insertError: uniqueViolation() }),
            ORG,
          ),
        { path },
      );

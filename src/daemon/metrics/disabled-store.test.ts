...
const nullMetrics = {} as AuthenticatedHostMetricsSample["metrics"];
...
it("DisabledServerMetricsStore queries return available:false", async () => {
  ...
  const series = await store.queryHostSeries({
    serverId: "srv-1",
    metrics: ["cpuUserPercent", "memoryTotalBytes"],
    from: "2026-01-01T00:00:00.000Z",
    to: "2026-01-01T01:00:00.000Z",
  });
  assertEquals(series.kind, "disabled");
  assertEquals(series.available, false);
  assertEquals(series.metrics, ["cpuUserPercent", "memoryTotalBytes"]);
  ...
  assertEquals(fleet.metrics, ["cpuUserPercent"]);
});

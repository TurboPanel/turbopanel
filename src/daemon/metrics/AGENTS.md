...
**Fleet latest snapshot** (`queryFleetHostSnapshot` / `GET …/servers/metrics/latest`): one AE/ClickHouse `GROUP BY server_id` over authorized ids with weighted (AE) / unit-weight (ClickHouse) averages for the fleet usage metric set (`cpuUsagePercent` + CPU breakdown + load averages + memory/swap). Used by the UI servers overview totals + usage bars. Cap `MAX_FLEET_SNAPSHOT_SERVERS` = 500.

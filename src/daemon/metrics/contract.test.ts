...
/**
 * Cross-repo parity: `contract.ts` is mirrored in the daemon repo
 * (`turbopaneld/src/metrics/contract.ts`). Read the peer file from the
 * co-located checkout (`TURBOPANEL_DAEMON_REPO` or `../turbopaneld` next to
 * this repo) and assert the wire contract matches exactly. CI only checks out
 * this repo, so the peer may be absent there — the daemon-side twin of this
...
async function readPeerContractSource(): Promise<string | null> {
  ...
  const root = override ||
    new URL("../../../../turbopaneld", import.meta.url).pathname;
  try {
    return await Deno.readTextFile(join(root, "src/metrics/contract.ts"));
  } catch (err) {
...
function parsePeerContract(
  source: string,
): { schemaVersion: number; metricKeys: string[]; keyParts: [string, string][] } {
...
it("HOST_METRIC_KEYS + METRICS_SCHEMA_VERSION + METRIC_KEY_PARTS match the daemon repo contract", async () => {

...
it("writeHostSample: exactly two writeDataPoints — core then extended", () => {
...
it("writeHostSample: indexes is authenticated serverId only on both parts", () => {
  const fake = createFakeDataset();
  const store = new CloudflareAnalyticsEngineServerMetricsStore(fake.dataset);
  const input = sample({
  ...
  for (const call of fake.calls) {
    assertEquals(call.indexes, ["aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"]);
  }
});
...
function sqlConfig(
  fetchImpl: NonNullable<CloudflareAnalyticsSqlConfig["fetch"]>,

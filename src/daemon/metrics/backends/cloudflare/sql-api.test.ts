...
import {
  ...
  buildHardwareProfileGenerationsSql,
  buildHostSeriesSql,
  buildHostSummarySql,
  ...
  parseHardwareProfileGenerationsRows,
  parseHostSummaryRow,
  parseSeriesRows,
  ...
  stripAeSentinel,
  sumExpressionForMetric,
  sumWeightExpressionForMetric,
...
it('weightedAvgExpressionForMetric: part-scoped, interval × sampling weighted, sentinel-safe', () => {
  ...
  assertEquals(expr.includes(`${intervalSecondsColumn()} * _sample_interval`), true)
  // Aggregation stays scoped to the part that owns the metric.
  assertEquals(
    expr.includes(`${blobColumn(AE_BLOB_PART_INDEX)} = ${quoteSqlString(AE_PART_CORE)}`),
    true
  )
...
it('sumWeightExpressionForMetric: part-scoped AE sampling weight, no intervalSeconds', () => {
  assertEquals(metricPart('caddyRequestsTotal'), AE_PART_TRAFFIC)
  const expr = sumWeightExpressionForMetric('caddyRequestsTotal')
  assertEquals(
    expr.includes(`${blobColumn(AE_BLOB_PART_INDEX)} = ${quoteSqlString(AE_PART_TRAFFIC)}`),
    true
  )
  assertEquals(expr.includes('_sample_interval'), true)
  // sum's weight is AE sampling only — never the sample's intervalSeconds.
  assertEquals(expr.includes(intervalSecondsColumn()), false)
})

it('sumExpressionForMetric: sentinel-safe value × AE sampling weight, no intervalSeconds', () => {
  const expr = sumExpressionForMetric('caddyRequestsTotal')
  assertEquals(expr.includes(aeMissingMetricSentinelSql()), true)
  assertEquals(expr.includes('caddyRequestsTotal_value'), true)
  assertEquals(expr.includes('caddyRequestsTotal_weight'), true)
  // Never double-weighted by intervalSeconds — a delta is already a total
  // for its own collection interval (see weighted-average's asymmetry).
  assertEquals(expr.includes(intervalSecondsColumn()), false)
})
...
it('hostEventDiscriminatorPredicates derive event type + schema version', () => {
  ...
  assertEquals(
    predicates[0],
    `${blobColumn(AE_BLOB_EVENT_TYPE_INDEX)} = ${quoteSqlString(AE_METRICS_EVENT_TYPE)}`
  )
  assertEquals(
    predicates[1],
    `${blobColumn(AE_BLOB_SCHEMA_VERSION_INDEX)} = ${quoteSqlString(
...
it('buildHostSummarySql: quoted serverId + core-part sample_count/latest_at', () => {
  ...
  // latest_at is scoped to the core part only — never a raw max over every
  // part row — so an orphan sensors/traffic write can't advance it past the
  // newest core-backed sample that sample_count actually counts.
  assertEquals(
    sql.includes(
      `max(if(${blobColumn(
        AE_BLOB_PART_INDEX
      )} = ${quoteSqlString(AE_PART_CORE)}, toUnixTimestamp(timestamp), 0)) AS latest_at`
    ),
    true
  )
...
it('buildFleetHostSnapshotSql: IN-list + GROUP BY server + weighted metrics', () => {
  ...
  assertEquals(metrics, ['cpuUserPercent', 'memoryAvailableBytes', 'swapFreeBytes'])
  ...
  assertEquals(
    sql.includes(`${weightedAvgNumeratorForMetric('cpuUserPercent')} AS cpuUserPercent_num`),
    true
  )
  assertEquals(
    sql.includes(`${weightedAvgDenominatorForMetric('cpuUserPercent')} AS cpuUserPercent_den`),
    true
  )
...
it('buildHostSeriesSql: recombines parts into logical samples keyed by (index1, timestamp)', () => {
  ...
  assertEquals(sql.includes(`toUnixTimestamp(timestamp) AS sample_ts`), true)
  assertEquals(sql.includes('GROUP BY server_id, sample_ts'), true)
  assertEquals(sql.includes(`${sampleCountExpression()} AS core_weight`), true)
...
it('host read + status SQL right edge is exclusive — canonical half-open [from, to)', () => {
  ...
  const statements = [
    buildHostSeriesSql({ serverId: SERVER_ID, metrics: ['cpuUserPercent'], from, to }, opts).sql,
...
it('buildRecentlyActiveServerIdsSql: fleet-wide metrics discriminators, core part only', () => {
  ...
  assertEquals(
    sql.includes(`${blobColumn(AE_BLOB_PART_INDEX)} = ${quoteSqlString(AE_PART_CORE)}`),
    true
  )
  ...
  await queryRecentlyActiveServerIds(
    {
      ...
      fetch: (_url, init) => {
        ...
        return Promise.resolve(new Response(envelopedSqlResponse([]), { status: 200 }))
      },
    },
    {
      sinceSeconds: AE_LIVENESS_WINDOW_SECONDS,
...
it('CloudflareAnalyticsSqlClient.executeSql posts raw SQL and unwraps the envelope', async () => {
  ...
  const client = new CloudflareAnalyticsSqlClient({
    ...
    fetch: async (input, init) => {
      ...
      return new Response(envelopedSqlResponse([{ answer: 42 }]), { status: 200 })
    },
  })
  ...
  assertThrows(
    () =>
      parseCloudflareV4SqlResponse({
        success: false,
        errors: [],
        result: { error: 'table turbopanel_server_host_metrics does not exist' },
      }),
    Error,
    'table turbopanel_server_host_metrics does not exist'
  )
})
  ...
  const result = await queryHostSummaryViaSqlApi(
    {
      ...
      fetch: async () =>
        new Response(
          envelopedSqlResponse([{ sample_count: 3, latest_at: '2026-01-15 12:34:56' }]),
          { status: 200 }
        ),
    },
    {
...
it('parseAeLatestAtMs accepts unix seconds, milliseconds, and space-separated UTC', () => {
  ...
  assertEquals(parseAeLatestAtMs('2026-01-15 12:34:56'), Date.parse('2026-01-15T12:34:56Z'))
  assertEquals(
...
it('parseHostSummaryRow normalizes latest_at when samples exist', () => {
  ...
  assertEquals(
    parseHostSummaryRow({ sample_count: 0, latest_at: '2026-01-15 12:34:56' }).latestAt,
    null
  )
})
...
it('quoteServerIdInList rejects empty and over-cap lists', () => {
  assertThrows(() => quoteServerIdInList([]), TypeError, 'serverIds must be non-empty')
  assertThrows(
    () =>
      quoteServerIdInList(Array.from({ length: MAX_FLEET_SNAPSHOT_SERVERS + 1 }, () => SERVER_ID)),
    TypeError,
...
it('quoteServerIdInList dedupes and validates UUID literals', () => {
  ...
  assertThrows(() => quoteServerIdInList(['not-a-uuid']), TypeError, 'invalid serverId')
})
...
it('resolveTruncatedStatusEvents marks the cap from raw row count', () => {
  ...
  assertEquals(over.knownUntilMs, fromMs + (MAX_STATUS_EVENTS - 1) * 1000)
...
it('parseCloudflareV4SqlResponse maps remaining envelope and error shapes', () => {
  assertThrows(() => parseCloudflareV4SqlResponse([]), TypeError, 'not a JSON object')
...
// write-path builders (`buildPartDataPoint` / `buildStatusDataPoint`), so
// these tests prove logical-sample recombination and half-open range
// behavior end to end — string assertions cannot.
// ---------------------------------------------------------------------------
...
async function withAeSqlFixture(run: (fixture: AeSqlFixture) => Promise<void>): Promise<void> {
  const instance = await DuckDBInstance.create(':memory:')
  const connection = await instance.connect()
  try {
    await connection.run("SET TimeZone = 'UTC'")
    // Documented AE SQL functions the DuckDB dialect lacks.
    await connection.run('CREATE MACRO intDiv(a, b) AS CAST(floor(a / b) AS BIGINT)')
    await connection.run('CREATE MACRO toUnixTimestamp(ts) AS CAST(epoch(ts) AS BIGINT)')
    await connection.run('CREATE MACRO toDateTime(u) AS epoch_ms(CAST(u AS BIGINT) * 1000)')
    await connection.run('CREATE MACRO argMax(arg, val) AS arg_max(arg, val)')
    ...
    await run({
      async insertPoint(point, atIso, sampleInterval = 1) {
        const values = [
          ...
          `TIMESTAMP ${quoteSqlString(atIso.replace('T', ' ').replace('Z', ''))}`,
          String(sampleInterval),
          ...point.doubles.map((d) => String(d)),
          ...point.blobs.map((b) => quoteSqlString(b)),
        ]
        await connection.run(`INSERT INTO ${AE_DATASET_NAME} VALUES (${values.join(', ')})`)
      },
      async query(sql) {
...
it('executed series SQL: one-sided part rows never skew logical samples', async () => {
  await withAeSqlFixture(async (fixture) => {
    ...
    await fixture.insertPoint(buildPartDataPoint(full, AE_PART_CORE), full.at)
    await fixture.insertPoint(buildPartDataPoint(full, AE_PART_EXTENDED), full.at)
    ...
    await fixture.insertPoint(buildPartDataPoint(coreOnly, AE_PART_CORE), coreOnly.at)
    ...
    await fixture.insertPoint(buildPartDataPoint(extendedOnly, AE_PART_EXTENDED), extendedOnly.at)
    ...
    await fixture.insertPoint(buildPartDataPoint(secondBucket, AE_PART_CORE), secondBucket.at)
    ...
    const { points, sampleCount } = parseSeriesRows(metrics, rows, bucketSeconds)
...
it('executed fleet snapshot SQL: sample_count/latest_at track logical samples only', async () => {
  ...
  await withAeSqlFixture(async (fixture) => {
    ...
    await fixture.insertPoint(buildPartDataPoint(fullA, AE_PART_CORE), fullA.at)
    await fixture.insertPoint(buildPartDataPoint(fullA, AE_PART_EXTENDED), fullA.at)
    const orphanA = execHostSample({ atMs: EXEC_T0 + 540_000 })
    await fixture.insertPoint(buildPartDataPoint(orphanA, AE_PART_EXTENDED), orphanA.at)
    ...
    await fixture.insertPoint(buildPartDataPoint(boundaryA, AE_PART_CORE), boundaryA.at)
    await fixture.insertPoint(buildPartDataPoint(boundaryA, AE_PART_EXTENDED), boundaryA.at)
    ...
    await fixture.insertPoint(buildPartDataPoint(orphanB, AE_PART_EXTENDED), orphanB.at)
    ...
    const servers = parseFleetHostSnapshotRows(metrics, await fixture.query(sql))
...
it('executed series + summary SQL: sample exactly at `to` belongs to the next range only', async () => {
  await withAeSqlFixture(async (fixture) => {
    ...
    await fixture.insertPoint(buildPartDataPoint(sample, AE_PART_CORE), sample.at)
    await fixture.insertPoint(buildPartDataPoint(sample, AE_PART_EXTENDED), sample.at)
    ...
    const earlierSummary = parseHostSummaryRow(
      (await fixture.query(buildHostSummarySql({ serverId: SERVER_ID, ...earlier }, opts)))[0]
    )
    assertEquals(earlierSummary, { sampleCount: 0, latestAt: null })
    const laterSummary = parseHostSummaryRow(
      (await fixture.query(buildHostSummarySql({ serverId: SERVER_ID, ...later }, opts)))[0]
    )
...
it('executed summary SQL: a newer orphan sensors/traffic row does not advance latest_at past the newest core sample', async () => {
  await withAeSqlFixture(async (fixture) => {
    // One complete logical sample (core + extended)...
    const fullSample = execHostSample({
      atMs: EXEC_T0 + 60_000,
      metrics: { cpuUserPercent: 40 },
    })
    await fixture.insertPoint(buildPartDataPoint(fullSample, AE_PART_CORE), fullSample.at)
    await fixture.insertPoint(buildPartDataPoint(fullSample, AE_PART_EXTENDED), fullSample.at)
    // ...then a newer sensors-only orphan whose core write never landed
    // (each part is an independent fire-and-forget writeDataPoint call).
    const orphan = execHostSample({
      atMs: EXEC_T0 + 300_000,
      parts: ['core', 'extended', 'sensors'],
      metrics: { cpuTemperatureCelsius: 55 },
    })
    await fixture.insertPoint(buildPartDataPoint(orphan, AE_PART_SENSORS), orphan.at)

    const opts = {
      dataset: AE_DATASET_NAME,
      maxRangeSeconds: AE_DEFAULT_MAX_RANGE_SECONDS,
    }
    const summary = parseHostSummaryRow(
      (
        await fixture.query(
          buildHostSummarySql(
            {
    ...
    // sample_count still counts only the core-backed logical sample...
    assertEquals(summary.sampleCount, 1)
    // ...and latest_at must not advance to the newer orphan's timestamp.
    assertEquals(summary.latestAt, isoAt(EXEC_T0 + 60_000))
  })
})
...
it('executed status events SQL: event exactly at `to` belongs to the next range only', async () => {
  await withAeSqlFixture(async (fixture) => {
    ...
    const earlierRows = await fixture.query(
      buildStatusEventsSql(
    ...
    const laterRows = await fixture.query(
      buildStatusEventsSql(
        {
          serverId: SERVER_ID,
          from: isoAt(boundaryMs),
...
it('buildHostSeriesSql: descriptor dispatch — argMax for last, MAX for max, num/den for weighted', () => {
  ...
  assertEquals(
    sql.includes(
      `${rawValueExpressionForMetric('systemStorageTotalBytes')} AS systemStorageTotalBytes_value`
    ),
    true
  )
  assertEquals(
    sql.includes(
      `${lastValueExpressionForMetric('systemStorageTotalBytes')} AS systemStorageTotalBytes`
    ),
    true
  )
  // max metric: plain MAX over the raw value column.
  assertEquals(
    sql.includes(`${maxValueExpressionForMetric('uptimeSeconds')} AS uptimeSeconds`),
    true
  )
  ...
  assertEquals(
    sql.includes(`${weightedAvgIntervalSecondsExpression()} AS avg_interval_seconds`),
    true
  )
  assertEquals(sql.includes('avg(interval_seconds)'), false)
  assertEquals(sql.includes(`${intervalSecondsColumn()}, 0.0)) AS interval_seconds`), true)
})

it('buildHostSeriesSql: descriptor dispatch — sum metric routes through sumExpressionForMetric', () => {
  const { sql } = buildHostSeriesSql(
    {
      serverId: SERVER_ID,
      metrics: ['caddyRequestsTotal'],
      from: '2026-01-01T00:00:00.000Z',
      to: '2026-01-01T01:00:00.000Z',
      resolutionSeconds: 60,
    },
  ...
  // Subquery grain: sentinel-safe raw value paired with its own weight —
  // never a num/den pair (that shape is weighted-average only).
  assertEquals(
    sql.includes(
      `${rawValueExpressionForMetric('caddyRequestsTotal')} AS caddyRequestsTotal_value`
    ),
    true
  )
  assertEquals(
    sql.includes(
      `${sumWeightExpressionForMetric('caddyRequestsTotal')} AS caddyRequestsTotal_weight`
    ),
    true
  )
  assertEquals(sql.includes('caddyRequestsTotal_num'), false)
  assertEquals(sql.includes('caddyRequestsTotal_den'), false)
  // Outer bucket select: the sum aggregate, never a weighted-average divide.
  assertEquals(
    sql.includes(`${sumExpressionForMetric('caddyRequestsTotal')} AS caddyRequestsTotal`),
    true
  )
  assertEquals(sql.includes('SUM(caddyRequestsTotal_num) / SUM(caddyRequestsTotal_den)'), false)
})

it('executed series SQL: sum metric totals per-interval deltas weighted by AE sampling only', async () => {
  await withAeSqlFixture(async (fixture) => {
    // Two traffic-part rows in the same bucket: 100 requests at
    // _sample_interval=1, then 50 requests at _sample_interval=3 (AE
    // retained one row standing in for three sampled-away rows).
    const first = execHostSample({
      atMs: EXEC_T0 + 60_000,
      parts: ['core', 'extended', 'traffic'],
      metrics: { caddyRequestsTotal: 100 },
    })
    await fixture.insertPoint(buildPartDataPoint(first, AE_PART_CORE), first.at)
    await fixture.insertPoint(buildPartDataPoint(first, AE_PART_EXTENDED), first.at)
    await fixture.insertPoint(buildPartDataPoint(first, AE_PART_TRAFFIC), first.at, 1)

    const second = execHostSample({
      atMs: EXEC_T0 + 90_000,
      parts: ['core', 'extended', 'traffic'],
      metrics: { caddyRequestsTotal: 50 },
    })
    await fixture.insertPoint(buildPartDataPoint(second, AE_PART_CORE), second.at)
    await fixture.insertPoint(buildPartDataPoint(second, AE_PART_EXTENDED), second.at)
    await fixture.insertPoint(buildPartDataPoint(second, AE_PART_TRAFFIC), second.at, 3)
    ...
    const rows = await fixture.query(sql)
    const { points } = parseSeriesRows(metrics, rows, bucketSeconds)
    assertEquals(points.length, 1)
    // 100*1 + 50*3 = 250 — AE-sampling-weighted, never intervalSeconds-weighted.
    assertEquals(points[0]!.values.caddyRequestsTotal, 250)
  })
})

it('executed series SQL: sum metric with no traffic rows in range reports null, not 0', async () => {
  await withAeSqlFixture(async (fixture) => {
    ...
    await fixture.insertPoint(buildPartDataPoint(s, AE_PART_CORE), s.at)
    await fixture.insertPoint(buildPartDataPoint(s, AE_PART_EXTENDED), s.at)

    const { sql, metrics, bucketSeconds } = buildHostSeriesSql(
      {
        serverId: SERVER_ID,
        metrics: ['cpuUserPercent', 'caddyRequestsTotal'],
        from: isoAt(EXEC_T0),
        to: isoAt(EXEC_T0 + 600_000),
        resolutionSeconds: 600,
      },
    ...
    assertEquals(points.length, 1)
    assertEquals(points[0]!.values.cpuUserPercent, 10)
    // No traffic row landed in this range at all — a `sum` metric with zero
    // contributing rows must report missing, never a fabricated "0 requests".
    assertEquals(points[0]!.values.caddyRequestsTotal, null)
  })
})
...
it('executed series SQL: sampled (_sample_interval > 1) mixed-cadence rows weight cadence and coverage', async () => {
  await withAeSqlFixture(async (fixture) => {
    ...
    await fixture.insertPoint(buildPartDataPoint(slow, AE_PART_CORE), slow.at, 2)
    await fixture.insertPoint(buildPartDataPoint(slow, AE_PART_EXTENDED), slow.at, 2)
    const fast = execHostSample({
      atMs: EXEC_T0 + 120_000,
      intervalSeconds: 20,
      metrics: { cpuUserPercent: 50 },
    })
    await fixture.insertPoint(buildPartDataPoint(fast, AE_PART_CORE), fast.at, 6)
    await fixture.insertPoint(buildPartDataPoint(fast, AE_PART_EXTENDED), fast.at, 6)
    ...
    const result = await queryHostSeriesViaSqlApi(
      {
        ...
        apiToken: 'token-xyz',
        fetch: async (_url, init) =>
          new Response(envelopedSqlResponse(await fixture.query(String(init?.body ?? ''))), {
            status: 200,
          }),
      },
      {
...
it('executed series SQL: last/max dispatch with sentinel-safe missing buckets', async () => {
  await withAeSqlFixture(async (fixture) => {
    ...
    const { points } = parseSeriesRows(metrics, await fixture.query(sql), bucketSeconds)
 ...
 * gapCount, resolutionSeconds, normalized timestamps, metric values, and the
 * optional-part / hardware-profile-generation metadata (see
 * `partsPresent`/`hardwareProfileGeneration`/`hardwareProfileGenerations` on
 * `HostSeriesPoint`/`HostSeriesResult`) — AE and DuckDB must agree on all of
 * it, not just the plotted values.
 */
function normalizeSeriesResultForParity(result: HostSeriesResult) {
  return {
    ...
    points: result.points.map((point) => ({
      ...
      partsPresent: point.partsPresent,
...
it('backend parity: identical samples aggregate identically on DuckDB and AE', async () => {
  ...
  const parityMetrics = ['cpuUserPercent', 'systemStorageTotalBytes', 'uptimeSeconds'] as const
...
// ---------------------------------------------------------------------------
// partsPresent / hardwareProfileGeneration parity — DuckDB tracks both today
// (see backends/duckdb/store.ts); the AE SQL API read path previously
// dropped this metadata entirely, hiding hardware charts and never
// segmenting reassignments for Analytics-Engine-backed hosts.
// ---------------------------------------------------------------------------

it('buildHostSeriesSql: selects per-part presence flags and the raw generation blob', () => {
  const { sql } = buildHostSeriesSql(
    {
      serverId: SERVER_ID,
      metrics: ['cpuUserPercent'],
  ...
  for (const part of ['core', 'extended', 'sensors', 'traffic']) {
    assertEquals(sql.includes(`AS ${part}_present`), true)
  }
  assertEquals(sql.includes('hardware_profile_generation_raw'), true)
  assertEquals(sql.includes('MIN(hardware_profile_generation_raw) AS hw_gen_min'), true)
  assertEquals(sql.includes('MAX(hardware_profile_generation_raw) AS hw_gen_max'), true)
  assertEquals(sql.includes(blobColumn(6)), true) // AE_BLOB_HARDWARE_PROFILE_GENERATION_INDEX
})

it('buildHardwareProfileGenerationsSql: scoped to the core part, GROUP BY generation', () => {
  const sql = buildHardwareProfileGenerationsSql(
    {
      serverId: SERVER_ID,
      metrics: ['cpuUserPercent'],
      from: '2026-01-01T00:00:00.000Z',
  ...
  assertEquals(sql.includes(`index1 = ${quoteSqlString(SERVER_ID)}`), true)
  assertEquals(sql.includes(`${blobColumn(AE_BLOB_PART_INDEX)} = ${quoteSqlString(AE_PART_CORE)}`), true)
  assertEquals(sql.includes('GROUP BY generation'), true)
  for (const predicate of hostEventDiscriminatorPredicates()) {
    assertEquals(sql.includes(`AND ${predicate}`), true)
  }
})

it('parseSeriesRows: partsPresent from per-part flags, hardwareProfileGeneration null on mixed min/max', () => {
  const { points } = parseSeriesRows(
    ...
    [
      ...
      },
      {
        ...
        sensors_present: 1,
        traffic_present: 0,
        // A reassignment happened inside this bucket — mixed generations.
        hw_gen_min: '1',
        hw_gen_max: '2',
      },
      {
        bucket: 1_704_067_800,
        sample_count: 1,
        cpuUserPercent: 14,
        // No flag/generation columns at all — a bucket-shaped row missing
        // this metadata must degrade to "nothing known", never throw.
      },
    ],
    300
  )
  assertEquals(points.length, 3)
  assertEquals(points[0]!.partsPresent, ['core', 'extended'])
  assertEquals(points[0]!.hardwareProfileGeneration, 1)
  assertEquals(points[1]!.partsPresent, ['core', 'extended', 'sensors'])
  assertEquals(points[1]!.hardwareProfileGeneration, null)
  assertEquals(points[2]!.partsPresent, [])
  assertEquals(points[2]!.hardwareProfileGeneration, null)
})

it('executed series SQL: partsPresent unions across samples in a bucket, generation nulls out across a reassignment', async () => {
  await withAeSqlFixture(async (fixture) => {
    // Bucket 1 (t=60s): two core+extended samples at generation 1 — one of
    // them also declares sensors. Union of parts across the bucket, one
    // consistent generation.
    const coreExtended = execHostSample({
      atMs: EXEC_T0 + 60_000,
      metrics: { cpuUserPercent: 10 },
      hardwareProfileGeneration: 1,
    })
    await fixture.insertPoint(buildPartDataPoint(coreExtended, AE_PART_CORE), coreExtended.at)
    await fixture.insertPoint(buildPartDataPoint(coreExtended, AE_PART_EXTENDED), coreExtended.at)
    const withSensors = execHostSample({
      atMs: EXEC_T0 + 90_000,
      parts: ['core', 'extended', 'sensors'],
      metrics: { cpuUserPercent: 20, cpuTemperatureCelsius: 55 },
      hardwareProfileGeneration: 1,
    })
    await fixture.insertPoint(buildPartDataPoint(withSensors, AE_PART_CORE), withSensors.at)
    await fixture.insertPoint(buildPartDataPoint(withSensors, AE_PART_EXTENDED), withSensors.at)
    await fixture.insertPoint(buildPartDataPoint(withSensors, AE_PART_SENSORS), withSensors.at)

    // Bucket 2 (t=360s): a hardware reassignment happens mid-bucket — two
    // samples, different generations.
    const beforeReassign = execHostSample({
      atMs: EXEC_T0 + 360_000,
      metrics: { cpuUserPercent: 30 },
      hardwareProfileGeneration: 1,
    })
    await fixture.insertPoint(buildPartDataPoint(beforeReassign, AE_PART_CORE), beforeReassign.at)
    await fixture.insertPoint(buildPartDataPoint(beforeReassign, AE_PART_EXTENDED), beforeReassign.at)
    const afterReassign = execHostSample({
      atMs: EXEC_T0 + 420_000,
      metrics: { cpuUserPercent: 40 },
      hardwareProfileGeneration: 2,
    })
    await fixture.insertPoint(buildPartDataPoint(afterReassign, AE_PART_CORE), afterReassign.at)
    await fixture.insertPoint(buildPartDataPoint(afterReassign, AE_PART_EXTENDED), afterReassign.at)
    ...
    assertEquals(points.length, 2)
    assertEquals(points[0]!.partsPresent, ['core', 'extended', 'sensors'])
    assertEquals(points[0]!.hardwareProfileGeneration, 1)
    assertEquals(points[1]!.partsPresent, ['core', 'extended'])
    // Mixed generations inside the bucket — unknown, not a fabricated guess.
    assertEquals(points[1]!.hardwareProfileGeneration, null)

    const generationsSql = buildHardwareProfileGenerationsSql(
      {
        serverId: SERVER_ID,
        metrics: ['cpuUserPercent'],
        from: isoAt(EXEC_T0),
    ...
    const generations = parseHardwareProfileGenerationsRows(await fixture.query(generationsSql))
    assertEquals(generations, [1, 2])
  })
})

it('queryHostSeriesViaSqlApi: populates hardwareProfileGenerations from the distinct-generations query', async () => {
  const result = await queryHostSeriesViaSqlApi(
    {
      accountId: 'acct123',
      apiToken: 'token-xyz',
      fetch: async (_url, init) => {
        const body = String(init?.body ?? '')
        // The two queries are distinguished by shape — the bucket query
        // GROUPs BY bucket, the generations query GROUPs BY generation.
        if (body.includes('GROUP BY generation')) {
          return new Response(envelopedSqlResponse([{ generation: 1 }, { generation: 3 }]), {
            status: 200,
          })
        }
        return new Response(
          envelopedSqlResponse([
            { bucket: 1_704_067_200, sample_count: 2, cpuUserPercent: 10.5 },
          ]),
          { status: 200 }
        )
      },
  ...
  assertEquals(result.hardwareProfileGenerations, [1, 3])
})

...
function parseDimensions(
  ...
  if (raw.schemaVersion !== METRICS_SCHEMA_VERSION) {
    return {
      ok: false,
      reason: `metrics dimensions.schemaVersion must be ${METRICS_SCHEMA_VERSION}`,
    };
  }
  const daemonVersion = readBoundedString(raw.daemonVersion, "dimensions.daemonVersion");
  if (!daemonVersion.ok) return daemonVersion;
  const operatingSystem = readBoundedString(
    raw.operatingSystem,
    "dimensions.operatingSystem",
  );
  if (!operatingSystem.ok) return operatingSystem;
  const architecture = readBoundedString(raw.architecture, "dimensions.architecture");
  if (!architecture.ok) return architecture;
  const kernelRelease = readBoundedString(raw.kernelRelease, "dimensions.kernelRelease");
  ...
  if (raw.runtimeMode !== undefined) {
    const runtimeMode = readBoundedString(raw.runtimeMode, "dimensions.runtimeMode");

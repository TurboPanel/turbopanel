...
import {
  ...
  setServerMetricsStoreFactoryForTests,
} from "./cell/do.ts";
...
describe("DaemonCellObject", () => {
  beforeEach(() => {
    ...
    setServerMetricsStoreFactoryForTests(null);
  });
...
describe("DaemonCellObject host metrics", () => {
  afterEach(() => {
    setServerMetricsStoreFactoryForTests(null);
    setDaemonCellProjectionDbFactoryForTests(null);
  });
  ...
  it("writes one host sample with authenticated serverId and no SQLite/alarm churn", async () => {
    const prev = env.TURBOPANEL_DAEMON_DEBUG;
    env.TURBOPANEL_DAEMON_DEBUG = "1";
    try {
      const serverId = "test-srv-metrics-write";
      ...
      setServerMetricsStoreFactoryForTests(() => fake.store);
      ...
      const alarmBefore = await runInDurableObject(stub, async (_instance, state) => {
        return await state.storage.getAlarm();
      });
      ...
      expect(fake.writes[0]?.serverId).toBe(serverId);
      ...
      const alarmAfter = await runInDurableObject(stub, async (_instance, state) => {
        return await state.storage.getAlarm();
      });
  ...
  it("cold-wake metrics skips ensure-schema reads/writes and alarms", async () => {
    const prev = env.TURBOPANEL_DAEMON_DEBUG;
    env.TURBOPANEL_DAEMON_DEBUG = "1";
    try {
      const serverId = "test-srv-metrics-cold-wake";
      const { db } = createProjectionRecordingDb({
        connected: true,
        daemonStatus: "online",
      });
      ...
      // Simulate hibernation cold-wake: new DO instance on the same storage +
      // restored WebSocket attachment. Metrics must not call `#ensureSchema`
      // (no version read / DDL) and must not change alarms.
      const coldResult = await runInDurableObject(
        stub,
        async (_instance, state) => {
          ...
          await cold.webSocketMessage(
            sockets[0]!,
            JSON.stringify(validMetricsWire({
              serverId: "client-spoofed-id",
            })),
          );
          ...
          const ensure = diag.storageByCallSite["ensure-schema"] ?? {
            reads: 0,
            writes: 0,
          };
          const alarmAfter = await state.storage.getAlarm();
          return { ensure, alarmBefore, alarmAfter, diag };
        },
      );

      expect(fake.writes).toHaveLength(1);
      expect(fake.writes[0]?.serverId).toBe(serverId);
      expect(coldResult.ensure.reads).toBe(0);
      expect(coldResult.ensure.writes).toBe(0);
      expect(coldResult.diag.storageReads).toBe(0);
      expect(coldResult.diag.storageWrites).toBe(0);
      expect(coldResult.alarmAfter).toBe(coldResult.alarmBefore);
  ...
  it("malformed metrics keeps the socket open and skips store write", async () => {
    const serverId = "test-srv-metrics-malformed";
    const { db } = createProjectionRecordingDb({
      connected: true,
      daemonStatus: "online",
    });
  ...
  it("oversized metrics keeps the socket open and skips store write", async () => {
    const serverId = "test-srv-metrics-oversized";
    const { db } = createProjectionRecordingDb({
      connected: true,
      daemonStatus: "online",
    });

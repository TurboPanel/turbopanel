...
test('buildDeploymentSecretsRehydrate seals server-scoped secrets via full path', async () => {
  ...
  assertEquals(row.secretPlan.length, 4)
  assertEquals(row.variableMaterial.length, 2)
  const keys = row.variableMaterial.map((m) => m.key).sort((a, b) =>
    a.localeCompare(b)
  )
  assertEquals(keys, ['DB_PASS', 'PLAIN_SECRET'])
  for (const material of row.variableMaterial) {
    assertEquals(material.valueEnvelope.startsWith(ENVELOPE_PREFIX_DAEMON), true)
    assertEquals(material.composeServiceName, 'web')
  }
  const plain = row.variableMaterial.find((m) => m.key === 'PLAIN_SECRET')
  assertEquals(plain?.isLiteral, true)
  assertEquals(plain?.forBuild, false)
  assertEquals(plain?.forRuntime, true)
})

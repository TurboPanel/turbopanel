...
async function waitForWebSocketMessageAfterAttachVersion(
  ...
  while (Date.now() < deadline) {
    const raw = await waitForWebSocketMessage(
      ws,
  ...
  throw new Error(
    "timed out waiting for websocket message after attach version",
  );
}

/** Consume the attach version frame that the cell sends before outbox traffic. */
async function drainAttachVersionFrame(
  ws: WebSocket,
  timeoutMs = 5000,
): Promise<void> {
    ...
    update: () => ({
      set: (patch: Record<string, unknown>) => {
        ...
        if (patch.daemon !== undefined) {
          daemon = materializeDaemonJsonbWrite(daemon, patch.daemon) ?? daemon;
...
function statusFromPatch(
  ...
  if (hasStatusChangedAt) {
    result.statusChangedAt = patch
      .statusChangedAt as ServerDaemonStatus["statusChangedAt"];
  }
  return result;
}
...
describe("DaemonCellObject diagnostics", () => {
  ...
  it(
    "idle connected cell does not schedule recurring sweep alarm",
    async () => {
      ...
      await runInDurableObject(
        stub,
        async (instance: DaemonCellObject, state) => {
  ...
  it(
    "idle connected cell does not re-arm sweep alarm across repeated alarm() calls",
    async () => {
  ...
  it("setAlarm skips write when alarm target is unchanged", async () => {
    ...
    try {
      ...
      const writesBefore =
        diagBefore.storageByCallSite["schedule-alarm"]?.writes ?? 0;
      ...
      const writesAfter =
        diagAfter.storageByCallSite["schedule-alarm"]?.writes ?? 0;
  ...
  it("liveness wake is SQLite-free after construct", async () => {
    ...
    try {
      ...
      for (
        const site of ["constructor", "ensure-schema", "snapshot"] as const
      ) {
  ...
  it(
    "schema is version-stamped and cold wake does not grow ensure-schema writes",
    async () => {
      const prev = env.TURBOPANEL_DAEMON_DEBUG;
      env.TURBOPANEL_DAEMON_DEBUG = "1";
      try {
        ...
        const diagBeforeResp = await cellRpc(
          stub,
          serverId,
          "/rpc/diagnostics",
          {
        ...
        const diagAfterResp = await cellRpc(
          stub,
          serverId,
          "/rpc/diagnostics",
          {
...
describe.sequential("DaemonCellObject", () => {
  ...
  it("replays the latest recorded capability plan to a hosted daemon on reconnect", async () => {
    ...
    const { db } = createProjectionRecordingDb(
      { connected: false },
      undefined,
      [
        {
  ...
  it("projects connect geo header into metadata.geo", async () => {
    ...
    const stub = env.DAEMON_CELL.getByName(serverId);
    const { ws } = await openDaemonWebSocket(
      stub,
      serverId,
      crypto.randomUUID(),
      {
  ...
  it("projects disconnect to Postgres after websocket close", async () => {
    ...
    await waitFor(() => {
      expect(
        updateCalls.some((patch) => statusFromPatch(patch)?.connected === true),
      ).toBe(true);
    });
  ...
  it(
    "projects daemonBuild change on heartbeat when commit changes",
    async () => {
  ...
  it(
    "steady-state heartbeat performs no DB write or connection open",
    async () => {
  ...
  it(
    "heartbeat-only after coalesce window skips projection DB open",
    async () => {
  ...
  it(
    "webSocketError cleans up like close and marks the cell disconnected",
    async () => {
      ...
      await waitFor(async () => {
        const snapshotResponse = await cellRpc(
          stub,
          serverId,
          "/rpc/snapshot",
          {
  ...
  it(
    ...
    async () => {
      ...
      const secondMessagePromise = waitForWebSocketMessageAfterAttachVersion(
        second.ws,
      );
...
describe("createRequestAndWait expiry parity", () => {
  it(
    "timed-out createRequestAndWait reclaims outbox and cannot redeliver",
    async () => {
      ...
      expect(readBody.envelopes.some((entry) => entry.requestId === requestId))
        .toBe(false);
  ...
  it("expire-request RPC is idempotent and returns persisted expired record", async () => {
    ...
    const expireResponse = await cellRpc(
      stub,
      serverId,
      "/rpc/expire-request",
      {
...
describe("command-dispatch correlation", () => {
  ...
  it(
    "command-ack and command-outcome write requests only, not cell liveness",
    async () => {
      const prev = env.TURBOPANEL_DAEMON_DEBUG;
      env.TURBOPANEL_DAEMON_DEBUG = "1";
      try {
  ...
  it(
    "delivery-requeue poison path attributes storageByCallSite and marks dead",
    async () => {
  ...
  it(
    "steady-state heartbeat performs no cell-table write on auto-response path",
    async () => {
  ...
  it(
    "heartbeat skips projection when connect seeded in-memory debounce is fresh",
    async () => {
      const prev = env.TURBOPANEL_DAEMON_DEBUG;
      env.TURBOPANEL_DAEMON_DEBUG = "1";
      try {
  ...
  it("RPC detach clears runtime connected flag in snapshot", async () => {
    ...
    const disconnectedSnapshot = await cellRpc(
      stub,
      serverId,
      "/rpc/snapshot",
      {
  ...
  it(
    "closes websocket when inbound messages exceed the per-connection limit",
    async () => {
      ...
      const diagAfterResponse = await cellRpc(
        stub,
        serverId,
        "/rpc/diagnostics",
        {
  ...
  it(
    "closes websocket on oversized inbound frames before storage work",
    async () => {
      ...
      const diagBeforeResponse = await cellRpc(
        stub,
        serverId,
        "/rpc/diagnostics",
        {
  ...
  it("keeps the websocket open when the inbound type is unknown", async () => {
    const serverId = "test-srv-inbound-unknown-type";
    const stub = env.DAEMON_CELL.getByName(serverId);
    const { ws } = await openDaemonWebSocket(stub, serverId);
    let closed = false;
    ...
    expect(closeEvent.reason).toBe("policy_violation");
  }, 15_000);
});
...
describe("DaemonCellObject storageByCallSite attribution", () => {
  it(
    "attributes SQL ops across enqueue, outbound pump, inbound, list, expire, clear-update, purge",
    async () => {
      ...
      const clearResponse = await cellRpc(
        stub,
        serverId,
        "/rpc/clear-update-status",
        {
          method: "POST",
          body: JSON.stringify({}),
        },
  ...
  it("alarm cleanup attributes alarm call-site reads and writes", async () => {
    ...
    const diagBefore = await readDiagnostics(stub, serverId);
    const alarmWritesBefore = diagBefore.storageByCallSite["alarm"]?.writes ??
      0;
  ...
  it(
    "ArrayBuffer webSocketMessage heartbeats do not write cell rows",
    async () => {
  ...
  it(
    "policy-violation close on invalid frame skips inbound storage sites",
    async () => {
  ...
  it(
    "re-delivery enqueue refreshes delivery fields under enqueue call site",
    async () => {
      ...
      const diagAfter = await readDiagnostics(stub, serverId);
      expect(
        (diagAfter.storageByCallSite["enqueue"]?.writes ?? 0) -
          enqueueWritesBefore,
      ).toBeGreaterThan(0);
      assertNoMisattributedStorage(diagAfter.storageByCallSite);
...
describe("DaemonCellObject alarm / outbox / RPC branch coverage", () => {
  ...
  it(
    "alarm demotes stale sockets when auto-response age exceeds sweep window",
    async () => {
  ...
  it(
    "outbox send failure requeues with retry_at before poison threshold",
    async () => {

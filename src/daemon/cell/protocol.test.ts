...
Deno.test('wireMessageToInboundEnvelope maps inbound wire types', () => {
  ...
  assertEquals(
    ...
    { kind: 'dev-sync-result', requestId: 'r4', at, ok: true, error: undefined },
  )
  ...
  assertEquals(
    ...
    { kind: 'tunnel-token-result', requestId: 'r5', at, ok: false, error: 'nope' },
  )
...
Deno.test('outboundEnvelopeToWireMessage maps outbound kinds', () => {
  ...
  assertEquals(
    outboundEnvelopeToWireMessage({ ...base, kind: 'command', command: 'uptime' }),

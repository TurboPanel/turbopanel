...
import {
  ...
  parseServerGeo,
  type ServerGeo,
} from "../../features/geo/server-geo.ts";
...
export type ServerDaemonProjectionRead =
  & Omit<ServerDaemonProjection, "hostname" | "machineKey">
  & {
...
export function daemonBuildChanged(
  ...
  if (
    daemonBuild.builtAt !== undefined &&
    daemonBuild.builtAt !== existing?.builtAt
  ) {
    return true;
  }
  if (
    daemonBuild.channel !== undefined &&
    daemonBuild.channel !== existing?.channel
  ) {
    return true;
  }
  if (
    daemonBuild.version !== undefined &&
    daemonBuild.version !== existing?.version
  ) {
    return true;
  }
  return false;
...
function buildIdentityProjection(
  ...
  return {
    ...
    ...(current?.features !== undefined ? { features: current.features } : {}),
  };
}
...
function applyOnlineTrigger(
  ...
  const { currentProjection, existingStatus, now, existing, daemonBuildHint } =
    ctx;
  ...
  if (identityDue || !currentProjection) {
    ...
    if (daemonBuildHint) {
      nextProjection = {
        ...nextProjection,
        daemonBuild: mergeDaemonBuildPreserving(
          currentProjection,
          daemonBuildHint,
        ),
      };
...
function applyUpdateResetTrigger(
  ctx: ProjectionTriggerContext,
): ProjectionOutcome {
  const { currentProjection, existingStatus } = ctx;
  return {
...
function buildDaemonAndStatusColumnPatch(
  ...
  if (writeProjection || writeStatus) {
    ...
    // never stored in jsonb either — only dedicated columns. Features are
    // copied from the row at write time when that key is already present, so
    // a hello that landed between this read and this write is kept.
    patch.daemon = daemonProjectionColumnPatch(
      buildDaemonJsonbPatch(
...
function resolveStatusTransitionReason(
  ...
  if (
    trigger.kind === "online" || trigger.kind === "offline" ||
...
export async function listConnectedServerIdsFromProjection(
  ...
  const rows = await db.select({ id: server.id }).from(server).where(
...
function toProjectionRead(
  row: ProjectionDaemonRow,

...
test('reencryptAtRestSecrets reseals old tpsecret blobs and skips tpdaemon/current', async () => {
  ...
  const unknownVersionEnvelope = v1VariableEnvelope.replace(/\.1\./, '.99.')

  const schemaName = `reencrypt_fix_${crypto.randomUUID().replaceAll('-', '')}`
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      // Never commit the fixture schema or sweep writes.
...
test('reencryptAtRestSecrets does not overwrite a concurrent secret update', async () => {
  ...
  const schemaName = `reencrypt_race_${crypto.randomUUID().replaceAll('-', '')}`
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      // Newer concurrent envelope must remain; stale reseal must not win.

...
test("getAdminOpenApiSpec documents the tier catalogue on Workers only", () => {
  ...
  assertEquals(
    workers["x-tagGroups"].some((group) => group.name === "Billing"),
    true,
  );
  ...
  const deno = getAdminOpenApiSpec("https://localhost:8443", {
    runtime: "deno",
  }) as Spec;
  assertEquals(deno.tags.some((tag) => tag.name === "Tiers"), false);
  assertEquals(
    deno["x-tagGroups"].some((group) => group.name === "Billing"),
    false,
  );
  assertEquals(
    Object.keys(deno.paths).some((path) =>
      path.startsWith(`${ADMIN_API_PREFIX}/tiers`)
    ),
    false,
  );

  const sharedWorkers = getAdminOpenApiSpec("https://localhost:8443", {
    runtime: "workers",
  }) as Spec;
  assertEquals(sharedWorkers.tags.some((tag) => tag.name === "Tiers"), false);
  assertEquals(
    Object.keys(sharedWorkers.paths).some((path) =>
      path.startsWith(`${ADMIN_API_PREFIX}/tiers`)
    ),
    false,
  );
...
test("getAdminOpenApiSpec documents public URL and reencrypt paths", () => {
  ...
  assertEquals(
    (authProviderEntry?.properties?.isEnvOverridden as SchemaObject).type,
    "boolean",
  );
...
test("getAdminOpenApiSpec documents the git app collection", () => {
  ...
  assertExists(
    spec.paths[`${ADMIN_API_PREFIX}/forges/github/manifest/callback`],
  );
...
test("getAdminOpenApiSpec accepts devSurface option without changing core paths", () => {
  const withDev = getAdminOpenApiSpec("https://localhost:8443", {
    devSurface: true,
  }) as {
    paths: Record<string, unknown>;
  };
  ...
  assertEquals(
    Object.keys(withDev.paths).sort((a, b) => a.localeCompare(b)),
    Object.keys(withoutDev.paths).sort((a, b) => a.localeCompare(b)),
  );
});

 ...
 * writers and failovers. See src/db/AGENTS.md (Multi-node PostgreSQL
 * model).
 */
 ...
 * bookkeeping shape forces this audit (and src/db/AGENTS.md,
  ...
  if (createIdx === -1) {
    throw new TypeError(
      'drizzle-orm PgDialect.migrate no longer contains its bookkeeping CREATE TABLE — ' +
        're-audit public.migration and update src/db/AGENTS.md',
    )
  }

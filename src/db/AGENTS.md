...
`TURBOPANEL_DATABASE_URL` whose host/port/database/user are not that env's
`HYPERDRIVE` origin as read from wrangler.jsonc + the Cloudflare API or
`wrangler hyperdrive get` — fail closed when the origin cannot be read;
skipped entirely when `CLOUDFLARE_ENV` is unset.
...
| `../../scripts/check-deploy-env.mjs` | `pnpm deploy` gate: with `CLOUDFLARE_ENV` set, the migrate URL must be that env's Hyperdrive origin (host/port/database/user); fail closed; skipped when unset |

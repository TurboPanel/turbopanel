...
17. **`project`** (Drizzle export `project`)
   ...
   - `repository_id` is the one Git repository this project **is** (null =
     ...
     project's compose has to name it — see `src/features/compose/AGENTS.md` →
...
rename (helpers `src/features/schedule/task-records.ts`; client CRUD
`src/client/tasks/routes.ts`). `tag` / `marker` exist (helpers `src/features/servers/tag-records.ts`). `copy`,

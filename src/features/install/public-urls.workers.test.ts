...
describe("hostFromPublicUrlEntry", () => {
  ...
  it("extracts hostname from https and bare host entries", () => {
    ...
    expect(hostFromPublicUrlEntry("panel.example.com")).toBe(
      "panel.example.com",
    );
...
describe("publicUrlEntryToInstallOrigin", () => {
  ...
  it("accepts https origins in production (no allowance)", () => {
    ...
    expect(publicUrlEntryToInstallOrigin("https://panel.example.com:443")).toBe(
      "https://panel.example.com:8443",
    );
    expect(
      publicUrlEntryToInstallOrigin("https://panel.example.com:9443"),
    ).toBe("https://panel.example.com:8443");
  });
  ...
  it("rejects plaintext http origins", () => {
    expect(publicUrlEntryToInstallOrigin("http://panel.example.com"))
      .toBeNull();
    expect(publicUrlEntryToInstallOrigin("http://dev.example.com"))
      .toBeNull();
  });
  ...
  it("canonicalizes a custom https port to the Access :8443 address", () => {
  ...
  it("formats a portless bracketed IPv6 host once in the install origin", () => {
    expect(publicUrlEntryToInstallOrigin("https://[2001:db8::1]")).toBe(
...
describe("parsePublicUrlEntries", () => {
  ...
  it("accepts only https origins in production", () => {
    ...
    expect(parsed.ok).toBe(true);
    if (parsed.ok) {
      expect(parsed.urls).toEqual(["https://panel.example.com:8443"]);
    }
  });

  it("rejects plaintext http entries", () => {
    const parsed = parsePublicUrlEntries(["http://dev.example.com"]);
    expect(parsed.ok).toBe(false);
  });
  ...
  it("accepts IPv6 bare host entries with explicit ports", () => {
    ...
    expect(parsed.ok).toBe(true);
    if (parsed.ok) {
      expect(parsed.urls).toEqual(["[2001:db8::1]:8443"]);
    }
  });

  it("normalizes a portless bracketed IPv6 entry without doubling brackets", () => {
    const https = parsePublicUrlEntries(["https://[2001:db8::1]"]);
    expect(https.ok).toBe(true);
    if (https.ok) {
      expect(https.urls).toEqual(["https://[2001:db8::1]:8443"]);
    }
    const bare = parsePublicUrlEntries(["[2001:db8::1]"]);
    expect(bare.ok).toBe(true);
    if (bare.ok) {
      expect(bare.urls).toEqual(["[2001:db8::1]"]);
    }
  });
});

describe("parseInstallBaseUrl", () => {
  it("keeps a hosted https origin and dials :8443 for a bare host", () => {
    expect(parseInstallBaseUrl("https://panel.example.com")).toBe(
      "https://panel.example.com",
    );
    expect(parseInstallBaseUrl("https://turbopanel.app")).toBe(
  ...
  it("rejects plaintext http base URLs", () => {
...
describe("resolvePublicBaseUrl forwarded-host validation", () => {
  ...
  it("ignores plaintext http forwarded origins in production", async () => {
    ...
    // A forwarded http origin is not an install origin — fall through.

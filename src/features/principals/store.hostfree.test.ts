      ...
      from: () => ({
        innerJoin: () => ({
          innerJoin: () => ({
            innerJoin: () => ({
              where: () => ({
                limit: () => Promise.resolve([{ organizationId: "org" }]),
              }),
            }),
            where: () => ({
...
test("isServerPrincipalUsernameTaken short-circuits blank and hits/misses", async () => {
  ...
  assertEquals(
    await isServerPrincipalUsernameTaken(hit, "org", "AppUser"),
    true,
  );
...
test("rotatePrincipalPassword returns generated plaintext", async () => {
  ...
  const rotated = await rotatePrincipalPassword(
    db,
    dataEncryptionSecrets,
    "p1",
  );
...
test("createManagedPrincipal inserts sealed password and validates inputs", async () => {
  ...
  assertEquals(insertValues?.organizationId, "org-1");
  assertEquals(insertValues?.managedId, "m1");
  assertEquals(typeof insertValues?.password, "string");
  assertEquals(
    (insertValues?.password as string).startsWith("tpsecret."),
    true,
  );

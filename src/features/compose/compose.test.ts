...
test("yamlToComposeDocument preserves top-level key order and comments", () => {
  ...
  assertEquals(
    doc.presentation.comments.services?.keyBefore?.includes("Web frontend"),
    true,
  );
  ...
  assertEquals(
    roundTrip.indexOf("services:") < roundTrip.indexOf("networks:"),
    true,
  );
});
...
`;
  ...
  assertEquals(
    roundTrip.indexOf("# comment") > roundTrip.indexOf("services:"),
    true,
  );
  assertEquals(
    roundTrip.indexOf("# comment") < roundTrip.indexOf("nginx:"),
    true,
  );
...
`;
  ...
  assertEquals(
    doc.presentation.comments.services?.keyBefore?.includes("test"),
    true,
  );
  ...
  assertEquals(
    roundTrip.indexOf("# test") < roundTrip.indexOf("services:"),
    true,
  );
});
...
`;
  const doc = yamlToComposeDocument(source);
  assertEquals(
    doc.presentation.comments["services.uptime-kuma.ports[0]"]?.inline
      ?.includes(
  ...
  assertEquals(
    doc.presentation.comments["services.uptime-kuma.volumes[0]"]?.inline
      ?.includes(
  ...
  assertEquals(
    doc.presentation.comments["services.uptime-kuma.environment[0]"]?.inline
      ?.includes(
        "Set the timezone",
      ),
    true,
...
test("validateComposeDocument accepts ComposeDocument and null", () => {
  ...
  if (accepted.ok) {
    assertEquals(
      (accepted.document.data.services as Record<string, unknown>).api,
      {
        image: "node:22",
      },
    );
...
test("validateComposeDocument rejects services missing image/build", () => {
  ...
  if (result.ok) return;
  assertEquals(
    result.issues.some((issue) => issue.message.includes("image")),
    true,
  );
});
...
`);
  ...
  const services = merged.data.services as Record<
    string,
...
test("empty overlay inherits base", () => {
  ...
  assertEquals((merged.data.services as Record<string, unknown>).web, {
    image: "nginx",
  });
});
...
function placementServerId(
  document: ReturnType<typeof emptyComposeDocument>,
): unknown {
...
function documentWithPlacement(
  serverId: unknown,
): ReturnType<typeof emptyComposeDocument> {
  return {
    version: 1,
    data: {
      services: {},
...
test("stripComposePlacement removes placement-only extension", () => {
  ...
  assertEquals(
    stripped.presentation.keyOrder.includes(TURBOPANEL_EXTENSION_KEY),
    false,
  );
});
...
test("mergeComposeOverlay after stripComposePlacement ignores project pin", () => {
  ...
  assertEquals(
    composeDocumentToRuntimeYaml(merged).includes(projectPin),
    false,
  );
});
...
test("mergeComposeOverlay merges presentation metadata from overlay", () => {
  ...
  const merged = mergeComposeOverlay(base, overlay);
  assertEquals(
    merged.presentation.comments.services?.keyBefore,
    "base comment",
  );
  assertEquals(
    merged.presentation.comments.networks?.keyBefore,
    "overlay comment",
  );
...
test("mergeComposeOverlay deepMerge skips undefined overlay values", () => {
  ...
  const merged = mergeComposeOverlay(base, overlay);
  const web = (merged.data.services as Record<string, Record<string, unknown>>)
    .web!;
...
test("stripComposePlacementOption removes placement from options.compose", () => {
  ...
  assertEquals(
    placementServerId(
      options.compose as ReturnType<typeof emptyComposeDocument>,
    ),
    undefined,
  );

...
test("ports: long syntax distinct host_ip / protocol keep separate keys", () => {
  ...
  const ports = servicesOf(mergeComposeOverlay(base, overlay)).web
    .ports as unknown[];
...
test("volumes: dedup by container target", () => {
  ...
  const volumes = servicesOf(mergeComposeOverlay(base, overlay)).web
    .volumes as unknown[];
...
test("presentation: replaced port keeps overlay comment at replaced index", () => {
  ...
  assertEquals(
    merged.presentation.comments["services.web.ports[1]"],
    undefined,
  );
});
...
test("presentation: scalar dedup drops overlay comment for discarded entry", () => {
  ...
  assertEquals(
    Object.keys(merged.presentation.comments).sort((a, b) =>
      a.localeCompare(b)
    ),
...
`);
  ...
  assertEquals(
    merged.presentation.comments["services.web.ports[1]"]?.inline?.includes(
      "https",
    ),
    true,
  );
...
test("depends_on: list form keyed dedup with overlay winning values", () => {
  ...
  const dependsOn =
    servicesOf(mergeComposeOverlay(base, overlay)).web.depends_on;
...
test("networks at root level deep-merge across overlay", () => {
  ...
  const merged = mergeComposeOverlay(base, overlay);
  const front =
    (merged.data.networks as Record<string, Record<string, unknown>>).front;
...
test("root-level secrets mapping deep-merges unique keys", () => {
  ...
  const merged = mergeComposeOverlay(base, overlay);
  const secrets = merged.data.secrets as Record<
    string,
...
test("root-level configs mapping deep-merges unique keys", () => {
  ...
  const merged = mergeComposeOverlay(base, overlay);
  const configs = merged.data.configs as Record<
    string,
    Record<string, unknown>
  >;
...
test("merge unwraps a tagged base scalar when overlay visits the same leaf", () => {
  ...
  assertEquals(
    servicesOf(mergeComposeOverlay(base, overlay)).web.image,
    "httpd",
  );
});

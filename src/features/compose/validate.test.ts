...
test("assertComposeDocument returns normalized document on success", () => {
  ...
  assertEquals((doc.data.services as Record<string, unknown>).api, {
    image: "node:22",
  });
});
  ...
  if (!result.ok) {
    assertEquals(
      result.issues.some((issue) =>
        issue.message.includes("unknown compose tag")
      ),
      true,
    );
  }
...
`);
  assertEquals(
    validateComposeDocument(document, { layer: "overlay" }).ok,
    true,
  );

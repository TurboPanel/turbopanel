...
function applySecretMounts(
  ...
  if (plan.forRuntime) {
    ...
    if (runtimeEnv) {
      runtimeEnv[secretFileEnvKey(plan.target)] = secretContainerPath(
        plan.target,
      );
    }
  }
...
function applySecretAssignment(
  ...
  if (existing) {
    ...
    const material = state.secretMaterial.find((row) =>
      row.key === entry.key &&
      row.composeServiceName === state.composeServiceName
    );
    if (material) {
  ...
  state.secretMaterial.push(
    secretMaterialFrom(entry, state.composeServiceName),
  );
}
...
function unresolvedError(
  ...
  return {
    ...
    message:
      `Unresolved variable ${labeled} on ${state.composeServiceName}.${envKey}`,
    ref: labeled,
    composeServiceName: state.composeServiceName,
    envKey,
  };
...
function applyEntriesToService(
  ...
  const runtimeErr = scanMapForRefs(
    state,
    runtimeEnv,
    "runtime",
    runtimeEnv,

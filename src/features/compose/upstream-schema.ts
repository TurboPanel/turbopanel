...
import schemaJson from "./vendor/compose-spec.schema.json" with {
  type: "json",
};
...
function checkBounds(
  ...
  if (typeof schema.minimum === "number" && value < schema.minimum) {
    pushIssue(
      ...
      `${
        pathLabel(path)
      } must be >= ${schema.minimum} per the Compose Specification, got ${value}`,
    );
    return;
  }
  if (typeof schema.maximum === "number" && value > schema.maximum) {
    pushIssue(
      ...
      `${
        pathLabel(path)
      } must be <= ${schema.maximum} per the Compose Specification, got ${value}`,
    );
  }
}
...
function checkOneOf(
  ...
  if (matched > 1) {
    pushIssue(
      ...
      `${
        pathLabel(path)
      } matches more than one allowed shape in the Compose Specification`,
    );
    return;
  }
  ...
  pushIssue(
    ...
    `${
      pathLabel(path)
    } does not match any shape the Compose Specification allows`,
  );
}

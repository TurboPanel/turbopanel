...
function commentText(
  ...
  const raw =
    (node as { commentBefore?: string | null; comment?: string | null })[which];
...
function walkMapPresentation(
  ...
  for (const item of node.items) {
    ...
    collectKeyFormatting(
      item.key as Node | null | undefined,
      childPath,
      collector,
    );
    walkPresentation(
...
function documentCommentText(
  ...
  const raw =
    (doc as { commentBefore?: string | null; comment?: string | null })[which];
  if (typeof raw !== "string" || raw.length === 0) return undefined;
  return raw;
}
...
export function yamlToComposeDocument(source: string): ComposeDocument {
  ...
  return {
    ...
    presentation: {
      ...
      ...(presentation.blankLines
        ? { blankLines: presentation.blankLines }
        : {}),

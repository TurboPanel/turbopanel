...
test("mergeComposeLayers ordering: later scalars win, sequences append across three layers", () => {
  ...
  const web =
    (merged.data.services as Record<string, Record<string, unknown>>).web;
...
test("stripComposePlacementFromLayer + stripComposeTurbopanelExtensions", () => {
  ...
  const ext = noPlacement.data[TURBOPANEL_EXTENSION_KEY] as Record<
    string,
    unknown
  >;
  ...
  const web =
    (stripped.data.services as Record<string, Record<string, unknown>>).web;
...
test("stripSiteServicesFromLayer looks through tagged services", () => {
  ...
  const services = unwrapComposeTag(next.data.services) as Record<
    string,
    unknown
  >;
...
test("renameComposeVolumesInLayer rewrites !override volumes payload", () => {
  ...
  const volumes =
    (next.data.services as Record<string, Record<string, unknown>>)
      .web.volumes;

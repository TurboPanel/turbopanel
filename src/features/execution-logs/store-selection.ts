...
import type { S3ExecutionLogConfig } from './s3-store.ts'
import { EXECUTION_LOG_RETENTION_DAYS, type ExecutionLogStore } from './types.ts'

export type { R2BucketLike, S3ExecutionLogConfig }

export type DenoExecutionLogStoreFactories = {
  filesystem: (directory: string) => ExecutionLogStore
  s3: (config: S3ExecutionLogConfig) => ExecutionLogStore
}

let denoFactories: DenoExecutionLogStoreFactories | null = null

/** Composition root on Deno registers FS/S3 drivers so Workers never imports them. */
export function setDenoExecutionLogStoreFactories(
  next: DenoExecutionLogStoreFactories | null,
): void {
  denoFactories = next
}
...
export function resolveExecutionLogStore(
  ...
  if (driver === 's3') {
    if (isFullS3Config(input.deno?.s3) && denoFactories) {
      return denoFactories.s3(input.deno.s3)
    }
    warnOnce(
      'deno-missing-s3',
  ...
  return denoFactories.filesystem(directory)
}

...
export type OrganizationBillingState = Readonly<{
   ...
   * (`src/features/tiers/self-hosted-grant.ts`). It is an entitlement, not a
 ...
 * `src/features/tiers/self-hosted-grant-records.ts` so `OrganizationBillingState`
 ...
 * `tierQuantitiesFromState` in `src/features/tiers/assignment-records.ts`.
 */

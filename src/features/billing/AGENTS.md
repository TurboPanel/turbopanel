...
is the _only_ place that talks to Stripe; `src/features/billing/billing-records.ts` is the
...
`payer`, `subscription`, `seat` (`src/db/AGENTS.md`) — is the system of
...
goes through `timingSafeSecretEquals` from `src/features/git/gitlab-webhook.ts`; the
...
chosen: `src/features/tiers/assignment.ts` takes the servers in bind order (oldest
...
**The ladder is code; the row is a binding.** `src/features/tiers/ladder.ts` is the
...
`updateTierById` in `src/features/tiers/tier-records.ts`. The insert is **plain**, not
...
records as delivered; `src/app/surfaces.test.ts` pins the path). Two ways:
...
skipped: `src/features/tiers/self-hosted-grant.ts` holds one granted `SX` unit per
...
What the Deno bundle still _imports_ from `src/features/billing/` is the

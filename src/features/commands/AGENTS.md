...
`src/features/execution-logs/seal-on-terminal.ts` (terminal transitions fire from
...
`src/features/execution-logs/AGENTS.md`.
...
- Shared abstraction: `CommandQueue` interface in `src/features/commands/queue.ts`;
  ...
  `src/features/commands/envelope.ts` — small (ids + type + timestamps; no large
...
`processCommandEnvelope` in `src/features/commands/consumer.ts` is the single source
...
(`src/features/webhook-delivery/webhook-delivery-records.ts`) before any side effect runs, so a
...
`src/platform/deno/server.ts` and the Workers cron in `src/daemon/cell/offline-sweep.ts`.

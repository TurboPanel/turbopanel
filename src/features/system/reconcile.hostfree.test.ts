...
function createRecordingQueue(): CommandQueue & {
...
test("buildSystemReconcilePayload builds hosting/managed/self-host components", async () => {
  ...
  assertEquals(
    managed.components[0]?.composeServiceName,
    SYSTEM_PROXYSQL_COMPOSE_SERVICE_NAME,
  );
  assertEquals(
  ...
  assertEquals(
    selfHost.components.every((c) =>
      c.desired === "present" && c.role === "turbopanel"
    ),
    true,
  );
});
...
test("buildSystemReconcilePayload marks managed-ingress present for consumer-only servers", async () => {
  ...
  assertEquals(
    payloads[0]?.components[0]?.composeServiceName,
    SYSTEM_PROXYSQL_COMPOSE_SERVICE_NAME,
  );
});

...
function resolveSequenceStrategy(path: MergePath): SequenceStrategy {
  ...
  if (
    isServiceAttrPath(path, "secrets") || isServiceAttrPath(path, "configs")
  ) {
...
function portUniqueKey(entry: unknown): string | null {
  ...
  const protocol = typeof entry.protocol === "string"
...
function appendScalarDedup(
  ...
  for (let oi = 0; oi < overlay.length; oi++) {
    ...
    const key = typeof item === "string" || typeof item === "number" ||
        typeof item === "boolean"
      ? String(item)
      : JSON.stringify(item);
...
function mergeTaggedOverlay(overlay: unknown): MergeBranch {
  ...
  if (composeTagOf(overlay) === "reset") {
    return { hit: true, value: DELETE_KEY };
  }
...
function mergePresentation(
  ...
  const documentCommentBefore = base.documentCommentBefore ??
    overlay.documentCommentBefore;

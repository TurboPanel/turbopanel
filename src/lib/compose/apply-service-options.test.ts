...
test("applyServiceOptionsToComposeDocument sets container_name and deploy limits", () => {
    ...
    new Map([
      ["api", {
        ...
        resources: {
          ...
          memoryReservationBytes: 256_000_000,
        },
  ...
  const api =
    (result.document.data.services as Record<string, Record<string, unknown>>)
      .api!;
  ...
  assertEquals(
    (api.deploy as { resources: { limits: { cpus: string; memory: string } } })
      .resources.limits,
...
test("applyServiceOptionsToComposeDocument applies defaults across multi-service maps", () => {
  ...
  assertEquals(
    (services.alpha.deploy as { restart_policy: { max_attempts: number } })
      .restart_policy
      .max_attempts,
    2,
  );
  ...
  assertEquals(
    (services.zebra.deploy as { restart_policy: { max_attempts: number } })
      .restart_policy
      .max_attempts,
    10,
  );
...
test("applyResourcesToComposeService writes only provided limits", () => {
  ...
  assertEquals(
    (service.deploy as { resources: { limits: { cpus: string } } }).resources
      .limits.cpus,
    "0.5",
  );
});
...
test("applyResourcesToComposeService memory-only omits cpus keys", () => {
  ...
  assertEquals(
    (service.deploy as { resources: { limits: Record<string, string> } })
      .resources.limits,
    { memory: "64" },
  );
});
...
test("buildServiceOptionsMap skips invalid option rows", () => {
  const map = buildServiceOptionsMap([
    { composeServiceName: "api", options: { instances: 2 } },
    {
      ...
      options: { healthCheck: { policy: "invalid" } },
    },
...
test("renaming a container to the service id keeps the authored name as an alias", () => {
  ...
  const adminer =
    (result.document.data.services as Record<string, unknown>).adminer;
...
test("the organization default fills only services that declare no ceiling", () => {
  const doc = emptyComposeDocument();
  doc.data.services = {
    unbounded: { image: "nginx:alpine" },
    ownLimit: { image: "worker:1", mem_limit: 268_435_456 },
    ownDeployLimit: {
      image: "api:1",
      deploy: { resources: { limits: { cpus: "0.25" } } },
    },
    site: {
      "x-turbopanel": { serviceKind: "site", root: "public", principal: "app" },
    },
  };
  ...
  const result = applyServiceOptionsToComposeDocument(
    ...
    new Map(),
    undefined,
    { cpus: 1, memoryBytes: 1_073_741_824 },
  );
  const services = result.document.data.services as Record<string, unknown>;

  const unbounded = services.unbounded;
  assertServiceRecord(unbounded, "unbounded");
  assertEquals(unbounded.cpus, 1);
  assertEquals(unbounded.mem_limit, 1_073_741_824);

  // A service that set its own ceiling is never overridden — the default
  // fills a gap, it does not cap anyone.
  const ownLimit = services.ownLimit;
  assertServiceRecord(ownLimit, "ownLimit");
  assertEquals(ownLimit.mem_limit, 268_435_456);
  assertEquals(ownLimit.cpus, undefined);

  const ownDeployLimit = services.ownDeployLimit;
  assertServiceRecord(ownDeployLimit, "ownDeployLimit");
  assertEquals(ownDeployLimit.cpus, undefined);
  assertEquals(ownDeployLimit.mem_limit, undefined);

  // Host-native services run under systemd, not Docker.
  const site = services.site;
  assertServiceRecord(site, "site");
  assertEquals(site.cpus, undefined);
  assertEquals(site.mem_limit, undefined);
});

test("no organization default leaves every service exactly as authored", () => {
  const doc = emptyComposeDocument();
  doc.data.services = { web: { image: "nginx:alpine" } };
  for (const limits of [undefined, null, {}]) {
    ...
    const web = (result.document.data.services as Record<string, unknown>).web;
    assertServiceRecord(web, "web");
    assertEquals(web.cpus, undefined);
    assertEquals(web.mem_limit, undefined);
    // `deploy` still carries the restart policy this pass always writes; what
    // must be absent is a resources ceiling nobody asked for.
    const deploy = web.deploy as Record<string, unknown> | undefined;
    assertEquals(deploy?.resources, undefined);
  }
});

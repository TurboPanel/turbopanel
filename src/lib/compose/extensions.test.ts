...
import { composeTagOf, makeComposeTag, unwrapComposeTag } from "./tags.ts";
...
test("stripComposeTurbopanelExtensions removes top-level extension and keyOrder entry", () => {
  ...
  assertEquals(
    output.presentation.keyOrder.includes(TURBOPANEL_EXTENSION_KEY),
    false,
  );
  assertEquals(
...
test("stripComposeTurbopanelExtensions removes per-service x-turbopanel", () => {
  ...
  const output = stripComposeTurbopanelExtensions(input);
  const web =
    (output.data.services as Record<string, Record<string, unknown>>).web;
...
test("stripComposeTurbopanelExtensions looks through tagged services mapping", () => {
  ...
  const web = (unwrapComposeTag(output.data.services) as Record<
    string,

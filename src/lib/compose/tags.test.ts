...
import { composeDocumentToYaml, yamlToComposeDocument } from "./convert.ts";
...
`;
  ...
  const web =
    (doc.data.services as Record<string, Record<string, unknown>>).web;
  ...
  const web2 =
    (again.data.services as Record<string, Record<string, unknown>>).web;

...
test("buildApplicationModel interprets deploy: through the schedule reader", () => {
  const application = buildApplicationModel(MERGED);
  assertEquals(
    applicationService(application, "cache")?.deployment.replicas,
    3,
  );
  assertEquals(
    applicationService(application, "cache")?.deployment.mode,
    "replicated",
  );
  // No `deploy.replicas` and no instances fallback supplied.
  assertEquals(
    applicationService(application, "worker")?.deployment.replicas,
    1,
  );
});
...
test("buildApplicationModel represents a service whose body is not a mapping", () => {
  const application = buildApplicationModel(
    doc({ services: { broken: "nope" } }),
  );
...
test("buildResolvedApplication marks the local half of a scheduled plan", () => {
  ...
  const cache = resolved.services.find((entry) =>
    entry.serviceId === "svc-cache"
  );

...
test("renameComposeVolumes rewrites top-level keys and short/long refs", () => {
  ...
  const web = (result.data.services as Record<string, Record<string, unknown>>)
    .web!;
...
test("renameComposeVolumes leaves bind mounts and unlisted keys alone", () => {
  ...
  assertEquals(
    (result.data.services as Record<string, Record<string, unknown>>).web!
      .volumes,

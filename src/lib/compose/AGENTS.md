...
(`src/lib/schedule/`) → **`allocateEnvironmentContainers`** (pre-allocate
...
validated in `src/lib/compose/service-kind.ts` / `site.ts` and mirrored in the
...
(`src/lib/db/principal-alias-records.ts`).
...
`loadOrganizationRepositoryIds` (`src/lib/db/repository-records.ts`) once per
...
`source_referenced_by_compose`. `ui/src/lib/compose/service-kind.ts` + `lint.ts`

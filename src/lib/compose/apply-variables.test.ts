...
describe("apply-variables", () => {
  ...
  it("injects runtime non-secrets via .env interpolation", () => {
    ...
    const services = result.document.data.services as Record<
      string,
  ...
  it("rewrites {$secret} to Compose secrets and KEY_FILE", () => {
    ...
    assertEquals(
      env[secretFileEnvKey("SECRET")],
      secretContainerPath("SECRET"),
    );
    assertEquals(result.secretMaterial.length, 1);
    assertEquals(result.secretPlan[0]?.source, "api_secret");
    assertEquals(
      JSON.stringify(result.document.data).includes("super-secret"),
      false,
    );
  ...
  it("merges per-service entries over globals and preserves existing env", () => {
    ...
    assertEquals(api.environment, {
      EXISTING: "keep",
      GLOBAL: composeInterpolationRef(
        serviceEnvInterpolationKey("api", "GLOBAL"),
      ),
      API_ONLY: composeInterpolationRef(
        serviceEnvInterpolationKey("api", "API_ONLY"),
      ),
    });
    const worker =
      (result.document.data.services as Record<string, Record<string, unknown>>)
    ...
    assertEquals(worker.environment, {
      GLOBAL: composeInterpolationRef(
        serviceEnvInterpolationKey("worker", "GLOBAL"),
      ),
    });
  });
  ...
  it("rewrites build-arg secrets onto build.secrets", () => {
    ...
    const api =
      (result.document.data.services as Record<string, Record<string, unknown>>)
        .api!;
    assertEquals(
      (api.build as { args?: Record<string, string> }).args?.TOKEN,
      undefined,
    );
    const buildSecrets =
      (api.build as { secrets: Array<{ source: string }> }).secrets;
    ...
    assertEquals(
      JSON.stringify(result.document.data).includes("build-secret"),
      false,
    );
  });
  ...
  for (
    ...
    it(`resolves scoped {$${refToken}.KEY} via perServiceScopes (${scopeLabel})`, () => {
      ...
      const env = (result.document.data.services as Record<
        string,
        Record<string, unknown>
      >)
        .web.environment as Record<string, string>;
  ...
  it("inlines TURBOPANEL_* build args without env file interpolation", () => {
    ...
    const api =
      (result.document.data.services as Record<string, Record<string, unknown>>)
        .api!;
    assertEquals(
      (api.build as { args: Record<string, string> }).args
        .TURBOPANEL_SERVICE_ID,
      "01989d42-9adb-7e65-bc2e-f38792c53691",
    );
    assertEquals(
      result.envFileContent.includes("TURBOPANEL_SERVICE_ID"),
      false,
    );
  });
  ...
  it("escapes literal TURBOPANEL_* inline values in environment", () => {
    ...
    assertEquals(
      result.envFileContent.includes("TURBOPANEL_PROJECT_ID"),
      false,
    );
  });

...
test("a wrong type is a blocking structural error", () => {
  const issues = lintWithoutResourceAdvisory(`services:
  web:
    image: nginx:alpine
    healthcheck:
      test: ["CMD", "true"]
...
test("a pattern is enforced on a real value and stands down for a placeholder", () => {
  const bad = lintWithoutResourceAdvisory(`services:
  web:
    image: nginx:alpine
...
`);
    ...
    assertEquals(
      resolveReplicaPolicy({ deploy: { replicas: testCase.value } }, 4)
        .replicas,
      4,
    );
  });

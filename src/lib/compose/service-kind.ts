...
import { validatePhpPoolSetting, validatePhpSetting } from "../php-settings.ts";
import {
...
function readNodePackageManager(
  value: unknown,
...
function applyContentExtensionFields(
  ...
  if (isPrincipalAlias(principalAlias)) {
    extension.principal = principalAlias.trim();
  }
...
export function readServiceTurbopanelExtension(
  ...
  return parseServiceTurbopanelExtension(
    service[TURBOPANEL_SERVICE_EXTENSION_KEY],
  );
}
...
function kindFieldIssue(
  ...
  return {
    path: `${basePath}.${field}`,
    message: `${field} is only valid when serviceKind is ${
      describeKinds(rule.kinds)
    }`,
  };
}
...
function validatePhpConsistency(
  ...
  for (
    ...
    if (block !== undefined) {
      issues.push(
        ...validatePhpDirectiveBlock(basePath, field, block, validate),
      );
    }
  }
...
function validateEngineConsistency(
  ...
  const issues = kindMembershipIssues(basePath, fields, [
    "engine",
    "sourceKind",
  ]);
  ...
  return [
    {
      path: `${basePath}.root`,
      message:
        'root must be a relative path without ".." (e.g. "public" or "www")',
    },
  ];
}
...
export function collectServiceTurbopanelValidationIssues(
  ...
  for (const [name, rawService] of Object.entries(services)) {
    ...
    issues.push(
      ...collectServiceExtensionValidationIssues(basePath, rawService),
    );
  }

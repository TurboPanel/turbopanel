...
test("coversAllHostnames requires every hostname to be covered", () => {
  ...
  assertEquals(
    coversAllHostnames(["*.example.com", "example.com"], [
      "a.example.com",
      "example.com",
    ]),
    true,
  ...
  assertEquals(
    coversAllHostnames(["*.example.com"], [
      "a.example.com",
      "b.example.com",
      "c.d.example.com",
    ]),
...
test("resolveTlsForHosting rejects missing, unready, or mismatched pins", () => {
  ...
  assertEquals(
    resolveTlsForHosting({
      ...
      hostnames: ["a.example.com"],
      candidates: [
        candidate("tls-1", ["*.example.com"], { status: "pending" }),
      ],
      now: NOW,
    }),
...
test("resolveTlsForHosting accepts a managed lets_encrypt pin without PEM dates", () => {
...
test("resolveTlsForHosting rejects managed pins that are not lets_encrypt", () => {
  assertEquals(
    resolveTlsForHosting({
      ...
      hostnames: ["app.example.com"],
      candidates: [
        candidate("tls-1", ["app.example.com"], { status: "managed" }),
      ],
      now: NOW,
    }),
    { ok: false, error: "pin_not_ready" },
...
test("resolveTlsForHosting falls back to internal for a revoked lets_encrypt pin", () => {
  const pinned: TlsCandidate = {
    id: "tls-le",
    source: "lets_encrypt",
    metadata: readyMeta(["app.example.com"], { status: "revoked" }),
    options: null,
  };
  assertEquals(
    resolveTlsForHosting({
...
test("resolveTlsForHosting still mismatches a revoked lets_encrypt pin that does not cover", () => {
  const pinned: TlsCandidate = {
    id: "tls-le",
    source: "lets_encrypt",
    metadata: readyMeta(["app.example.com"], { status: "revoked" }),
...
test("resolveTlsForHosting still reports pin_not_found for a deleted pin", () => {
  assertEquals(
    resolveTlsForHosting({
      pinId: "tls-missing",
      hostnames: ["app.example.com"],
...
test("resolveTlsForHosting still rejects a revoked pin that is not lets_encrypt", () => {
  assertEquals(
    resolveTlsForHosting({
      pinId: "tls-1",
      hostnames: ["app.example.com"],
      candidates: [
        candidate("tls-1", ["app.example.com"], { status: "revoked" }),
      ],
      now: NOW,
    }),
    { ok: false, error: "pin_not_ready" },

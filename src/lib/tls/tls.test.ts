...
describe("resolveTlsForHosting", () => {
  ...
  function candidate(
    ...
    opts?: {
      prefer?: number;
      status?: "ready" | "pending" | "expired" | "managed" | "revoked";
      source?: TlsCandidate["source"];
    },
  ): TlsCandidate {
    return {
      id,
  ...
  it("accepts a managed lets_encrypt pin without PEM readiness", () => {
    const result = resolveTlsForHosting({
      pinId: "pin-le",
      hostnames: ["api.example.com"],
      candidates: [
        candidate("pin-le", ["api.example.com"], {
          status: "managed",
          source: "lets_encrypt",
        }),
      ],
    ...
    assertEquals(result, { ok: true, tlsId: "pin-le", reason: "pin" });
  });
  ...
  it("falls back to internal when a lets_encrypt pin is revoked", () => {
    const result = resolveTlsForHosting({
      pinId: "pin-le",
      hostnames: ["api.example.com"],
      candidates: [
  ...
  it("still mismatches a revoked lets_encrypt pin that does not cover", () => {
    const result = resolveTlsForHosting({
      pinId: "pin-le",
      hostnames: ["other.test"],
      candidates: [
...
describe("mintSelfSignedCertificate", () => {
  ...
  it("honors custom validity and common name options", async () => {
    ...
    assertEquals(
      material.parsed.subject.includes("CN=custom.example.com"),
      true,
    );
  });
});

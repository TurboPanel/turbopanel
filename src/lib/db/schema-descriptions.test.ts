...
test('every shipped table has a group and a one-sentence summary', async () => {
  ...
  for (const table of snapshot.tables) {
    ...
    if (!description.summary?.trim()) failures.push(`${table.name}: empty summary`)
  }
  describeFailures(failures, `tables in ${snapshot.tag} without a description:`)
})
...
test('nothing describes a table or column that is not in the shipped schema', async () => {
  const snapshot = await latestSnapshot()
  const byName = new Map(snapshot.tables.map((t) => [t.name, new Set(t.columns.map((c) => c.name))]))
  const failures: string[] = []
  for (const [tableName, description] of Object.entries(SCHEMA_DESCRIPTIONS)) {
    const columns = byName.get(tableName)
    if (!columns) {
      failures.push(`${tableName}: not in ${snapshot.tag} (dropped or renamed?)`)
      continue
    }
    for (const columnName of Object.keys(description.columns)) {
      if (!columns.has(columnName)) failures.push(`${tableName}.${columnName}: no such column`)
      if (columnName in STANDARD_COLUMNS) {
        failures.push(`${tableName}.${columnName}: standard column — described once in STANDARD_COLUMNS`)
      }
    }
  }
...
test('descriptions are single sentences that survive SQL, GFM tables and MDX', () => {
  ...
  const check = (where: string, text: string) => {
    if (text !== text.trim()) failures.push(`${where}: leading/trailing whitespace`)
    if (text.length > MAX_DESCRIPTION_LENGTH) {
      failures.push(`${where}: ${text.length} chars (max ${MAX_DESCRIPTION_LENGTH})`)
    }
    const bad = FORBIDDEN_DESCRIPTION_CHARS.exec(text)
    if (bad) failures.push(`${where}: contains forbidden character ${JSON.stringify(bad[0])}`)
    if (!/[.!?)`']$/.test(text)) failures.push(`${where}: does not end a sentence`)
  }
  for (const [tableName, description] of Object.entries(SCHEMA_DESCRIPTIONS)) {
  ...
  for (const [name, text] of Object.entries(STANDARD_COLUMNS)) check(`STANDARD_COLUMNS.${name}`, text)

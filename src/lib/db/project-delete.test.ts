...
test("deleteProjectCascade rejects when containers are still active", async () => {
  ...
  const [proj] = await db
    .insert(project)
    .values({ name: "Running Project", workspaceId, organizationId })
...
test("deleteProjectCascade ignores a running ingress container", async () => {
  ...
  const [proj] = await db
    .insert(project)
    .values({ name: "Ingress Running Project", workspaceId, organizationId })
    .returning({ id: project.id });
  const projectId = proj!.id;
...
test("deleteProjectCascade removes project with no environments", async () => {
  ...
  const [proj] = await db
    .insert(project)
    .values({ name: "Empty Project", workspaceId, organizationId })
    .returning({ id: project.id });
  const projectId = proj!.id;
...
test("deleteProjectCascade removes children when containers are stopped", async () => {
  ...
  const [proj] = await db
    .insert(project)
    .values({ name: "Stopped Project", workspaceId, organizationId })
    .returning({ id: project.id });
  const projectId = proj!.id;
...
test("deleteProjectCascade removes tenancy edges before services", async () => {
  ...
  const [proj] = await db
    .insert(project)
    .values({ name: "Tenancy Cascade Project", workspaceId, organizationId })
    .returning({ id: project.id });
  const projectId = proj!.id;

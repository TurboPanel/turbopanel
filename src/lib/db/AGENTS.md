...
**Install (Deno):** A fresh DB has no org or superadmin. `src/client/authn/install-state.ts` `isInstanceInstalled()` is false until `completeInstanceInstall` creates org → **System workspace (`kind='system'`)** → team → superadmin → grants → **Default Workspace** (`kind='user'`) → colocated license. **"System"** is therefore a reserved workspace display name from first boot (**409** `workspace_name_in_use`). **`organization.slug`** stays **NULL** (reserved for a future feature). Org extras (e.g. logo URL) belong in **`organization.metadata`** — there is no `logo` column. Install sets **`email`** and **`role`** (on `user`) only — `display_name`, `username`, and `display_username` stay **NULL** until the user chooses them.

...
| `network` | `organization_id NOT NULL`; optional `datacenter_id` or `server_id` (CHECK: not both) | Org-owned network registry. **`kind`** CHECK `('datacenter', 'server', 'docker', 'vpn')`; nullable **`cidr`**. **`kind: docker`** rows register long-lived host Docker networks for compose `networks.*.external` — API requires **`options.dockerNetworkName`** (create/patch → **400** `docker_network_name_required` when missing/invalid). Project-internal compose networks are **not** stored here. `server_id` and `datacenter_id` FKs `ON DELETE RESTRICT` — deleting a datacenter is blocked while scoped networks remain (API returns **409** `datacenter_has_networks`). |
...
| `POST` | `/api/client/v1/networks` | org owner/manager; body requires `kind`; optional `datacenterId` xor `serverId`; `kind: docker` requires `options.dockerNetworkName` |

...
**Daemon key columns** (added in the simplified auth model): the `server` row now stores the one active daemon public key directly. No separate `serverkey` table exists for MVP.

| Column | Purpose |
|---|---|
| `daemon_key_id` (uuid, nullable) | Logical key identifier returned to the daemon as `keyId` on enrollment. Generated by the instance on `attachDaemonKeyToServer`. |
| `daemon_key_algorithm` (text, nullable) | Key algorithm; defaults to `Ed25519`. |
| `daemon_public_key` (jsonb, nullable) | Raw public JWK `{ crv, kty, x }`. |
| `daemon_key_fingerprint` (text, nullable, unique) | SHA-256 hex over the canonical public JWK — unique across all servers. |
| `daemon_key_created_at` (timestamptz, nullable) | When the key was attached (enrollment or re-enrollment). |
| `daemon_key_last_used_at` (timestamptz, nullable) | Updated on each successful auth/session handshake. |
| `daemon_key_revoked_at` (timestamptz, nullable) | Soft-delete. Non-null blocks new JWT issuance; existing JWTs remain valid until their 15-minute expiry. |

Re-enrollment with a valid license token overwrites all daemon key columns atomically. No historical key rows are kept for MVP. To revoke daemon auth, set `daemon_key_revoked_at` on the server row.

...
chain is intact. `--rebaseline` rewrites the manifest and bumps `generation`
(currently 8, matching the regeneration count above) — it is the one
carve-out, reserved for the pre-tag fold, which runs it in the fold commit
and flips `frozen` to `true`; from then on the CI wiring that lands with the
fold accepts appends only.
...
| `organization` | `organization`    | Tenant org. **`slug` dropped 2026-09-17** (migration 0020) — it was reserved, never written, and no feature ever claimed it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

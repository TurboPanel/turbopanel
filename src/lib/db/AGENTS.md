...
| `managed` | `project_id` and/or `server_id` (CHECK: at least one) | Catalog apps: `project_id` unique (partial index). Standalone managed DB/cache: `server_id` + `display_name`; engine/status/endpoints in **`metadata`** (`engine`, `status`, `rootPrincipalId`, `host`, `port`). Root creds via **`principal`** sealed as `tpsecret`. API: `GET/POST /api/client/v1/managed-services`. |

...
| **Networking** | `datacenter`, `network` (kinds `datacenter` / `docker` / `compose`), `ip` (scopes `public` / `datacenter` / `vpn` / `fabric`), `vpn` (owns `cidr`), `peer`, `fabric` (0–1 per org; TurboFabric on when present), `relay`, `span` |
...
| `span` | `span` | A `kind='compose'` network present on one server (local bridge subnet) |
...
organization → network → span                  (that network on one server + local subnet)
...
| `span` | `network_id` CASCADE + `server_id` RESTRICT | A `kind='compose'` network present on this server. Unique `(network_id, server_id)`; **`cidr`** is the server-local bridge subnet, carved from that relay's prefix. Includes `metadata`/`options`. |

...
`src/lib/db/enum-checks.test.ts`; add a value in both places or that test
...
enforced by `src/lib/db/migration-manifest.test.ts`). It pins every journal
...
it** (`src/lib/db/schema-state.ts`; Node twin `scripts/schema-state.mjs`,
...
| **Networking**    | `datacenter` (logical routing domain, not a building — a server may belong to several; `options.priority` / `options.trusted`), `network` (kinds `datacenter` / `docker` / `compose` / `managed` / `reserved` — `reserved` is an org-only, CIDR-required operator hole every CIDR write and fabric allocator routes around via `src/lib/net/cidr-collisions.ts`), `ip` (scopes `public` / `datacenter`), `fabric` (0–1 per org; TurboFabric on when present), `relay`, `subnet` |

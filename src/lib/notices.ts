...
export function walkDenoLockReachableKeys(
  ...
  while (queue.length > 0) {
    ...
    for (const hit of resolveDenoLockSpec(lock, spec)) {
      ...
      for (const dep of denoLockDependencies(hit.meta)) {
...
function denoLockDependencies(meta: DenoLockPackageMeta): readonly string[] {
...
/** Unprefixed `<name>@<version>` — jsr wins over npm, as before. */
...
function specifierValue(
  ...
  const match = Object.keys(specifiers).find((key) =>

...
test("parseSemver accepts release, pre-release, build metadata and a leading v", () => {
  assertEquals(parseSemver("0.1.0"), {
    major: 0,
    minor: 1,
    patch: 0,
    prerelease: [],
  });
  assertEquals(parseSemver("v1.2.3"), {
    major: 1,
    minor: 2,
    patch: 3,
    prerelease: [],
...
test("header names are lower-case, as Hono and fetch normalize them", () => {
  ...
  assertEquals(
    INSTANCE_REVISION_HEADER,
    INSTANCE_REVISION_HEADER.toLowerCase(),
  );
...
test("resolveDaemonCapabilities closes on an unknown peer and opens at the feature floor", () => {
  ...
  assertEquals(
    resolveInstanceCapabilities("0.1.1", { demo: "0.1.1" }).demo,
    true,
  );
  assertEquals(
    resolveInstanceCapabilities(null, { demo: "0.1.1" }).demo,
    false,
  );
});

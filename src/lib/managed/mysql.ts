...
function buildPlatformMycnf(
  ...
  const lines = [
    ...
    // Factor 1 must stay `*` so initdb can install auth_socket for docker-exec
    // admin. A plugin list (`caching_sha2_password,,`) forbids
    // `IDENTIFIED WITH auth_socket` and leaves `root@localhost` on
    // `MYSQL_ROOT_PASSWORD` — daemon SQL is credential-free (`mysql -u root`,
    // no `-p`) and then 1045s. `IDENTIFIED BY` still defaults to caching_sha2
    // for ProxySQL / managed-network users.
...
function buildInitdbSql(): string {
  return [
    ...
    // MySQL has no `INSTALL PLUGIN IF NOT EXISTS` (MariaDB-only). Initdb runs
    // once on an empty datadir; apply skips this when the plugin is already
    // loaded.
...
function buildRuntimeSpec(input: BuildRuntimeSpecInput): ManagedRuntimeSpec {
  ...
  const spec: ManagedRuntimeSpec = {
    ...
    exposure: {
      ...
      ...(settings.exposure.scope !== undefined

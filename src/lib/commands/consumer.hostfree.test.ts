...
test("resolveManagedIdFromPayload extracts ids and returns null on miss", () => {
  ...
  assertEquals(
    resolveManagedIdFromPayload("managed.apply", { managedId: "x" }),
    null,
  );
...
test("resolveManagedMemberIdFromFailedPayload reads member ids without inventing them", () => {
  ...
  assertEquals(
    resolveManagedMemberIdFromFailedPayload("managed.destroy", {}),
    null,
  );
  assertEquals(
    resolveManagedMemberIdFromFailedPayload("daemon.ping", {}),
    null,
  );
});
...
function createConsumerFakeDb(options: ConsumerFakeDbOptions = {}): {
  ...
  const dispatchPayload = options.dispatchPayload === undefined
    ? {}
      ...
      from: () => {
        const source = {
          ...
          where: () => {
            ...
            if (
              "id" in fields && "organizationId" in fields && "cidr" in fields
            ) {
            ...
            if ("serverId" in fields && "ordinal" in fields) {
              return queryResult(
                options.replicaMember ? [options.replicaMember] : [],
              );
            }
            ...
            if (
              ...
              return queryResult(
                options.replicaServerId
                  ? [{ serverId: options.replicaServerId }]
                  : [],
              );
            }
            ...
            if (
              "metadata" in fields && !("daemon" in fields) &&
              !("name" in fields)
            ) {
              return queryResult([{
            ...
            if (
              "id" in fields && !("daemon" in fields) && !("metadata" in fields)
            ) {
      ...
      values: (row: Record<string, unknown>) => {
        ...
        const inserted = {
          ...baseCommandRow({
            ...
            status: "queued",
            serverId: typeof row.serverId === "string"
              ? row.serverId
              : SERVER_ID,
          }),
        };
    ...
    update: () => ({
      set: (patch: Record<string, unknown>) => {
        ...
        if (
          ...
          (
            (patch.status === "ready" || patch.status === "stopped" ||
              patch.status === "failed") &&
            !("errorMessage" in patch) &&
            !("attempts" in patch) &&
            !("resultSummary" in patch) &&
        ...
        if (
          "publicKey" in patch || (typeof patch.metadata === "object" &&
            patch.metadata !== null &&
...
test("processCommandEnvelope retains the dispatch payload ~24h after a failure", async () => {
  ...
  assertEquals(
    transitions.some((t) =>
      t.status === "failed" && t.error === "daemon exploded"
    ),
    true,
  );
...
test("processCommandEnvelope fails cleanly when the dispatch payload is gone", async () => {
  ...
  assertEquals(
    transitions.some(
      (t) =>
        t.status === "failed" &&
        t.error === "Command dispatch payload unavailable",
    ),
    true,
  );
});
...
test("processCommandEnvelope deploy failure runs the failed-deploy side effect", async () => {
  ...
  assertEquals(
    fake.transitions.some((t) =>
      t.status === "failed" && t.error === "compose up failed"
    ),
    true,
  );
});
...
test("processCommandEnvelope deploy unexpected pending status fails the command", async () => {
  ...
  assertEquals(
    fake.transitions.some(
      (t) =>
        t.status === "failed" &&
        t.error === "Unexpected pending request status: acked",
    ),
    true,
  );
});
...
test("processCommandEnvelope fabric disable offline still runs the failed-fabric side effect", async () => {
  ...
  await processCommandEnvelope(
    fake.db,
    emptyRegistry(),
    typedEnvelope("server.fabric.reconcile"),
  );
...
test("processCommandEnvelope managed.apply failure marks managed rows failed", async () => {
  ...
  assertEquals(
    fake.transitions.some((t) =>
      t.status === "failed" && t.error === "apply exploded"
    ),
    true,
  );
});
...
test("processCommandEnvelope fabric success stamps a missing publicKey and desired hash", async () => {
  ...
  assertEquals(
    fake.relayUpdates.some((patch) =>
      typeof patch.metadata === "object" &&
      patch.metadata !== null &&
      (patch.metadata as { appliedPayloadHash?: string }).appliedPayloadHash ===
        "desired-hash-1"
    ),
    true,
  );
...
test("processCommandEnvelope managed.apply marks a follow-up failed when the queue rejects", async () => {
  ...
  assertEquals(
    fake.transitions.some((t) =>
      t.status === "failed" && t.error === "Command queue unavailable"
    ),
    true,
  );
});
...
test("processCommandEnvelope managed.promote without a replica pin still marks ready", async () => {
  ...
  assertEquals(fake.transitions.some((t) => t.status === "succeeded"), true);
  assertEquals(
    fake.managedUpdates.some((patch) =>
      patch.status === "ready" && !("serverId" in patch)
    ),
    true,
  );
});

...
function parseFirewallRule(value: unknown, index: number): FirewallCommandRule {
  ...
  if (!isString(value.scope) || !FIREWALL_SCOPES.has(value.scope)) {
    throw new Error(`rules[${index}].scope must be host or published`);
  }
  if (!isString(value.action) || !FIREWALL_ACTIONS.has(value.action)) {
    throw new Error(`rules[${index}].action must be accept, drop or reject`);
  }
  if (!isString(value.proto) || !FIREWALL_PROTOS.has(value.proto)) {
    throw new Error(`rules[${index}].proto must be tcp, udp or any`);
  }
  if (!isString(value.origin) || !FIREWALL_ORIGINS.has(value.origin)) {

...
| `POST`   | `/api/client/v1/organizations/:id/fabric/apply`            | session + manage             | Force-reconcile TurboFabric membership (`server.fabric.reconcile`) on every org relay; `{ enabled: false }` teardown is PUT disable. Returns `{ fabricId, interfaceName: 'tp0', results[] }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
`managed.restore`, `managed.promote`, `managed.ingress.reconcile`, and
...
solely because `skipped` is set would leave membership unconverged and re-enqueue
until timeout. Disable payloads (`{ enabled: false }`) skip the stamp — they tear
down `tp0`, routed bridges, `TP-FORWARD`, keys, and state. Reconcile failures
clear the applied hash so the next pass retries.
...
`hosting-ingress | managed-ingress | database | queue | analytics` — each
`SystemReconcileComponent` carries a `role` (`'ingress'` for the shared
per-server Traefik, `'turbopanel'` for managed-ingress ProxySQL and the
...
`<serviceId>-sql`, `database`/`queue`/`analytics` → bare `serviceId` (uuid
...
| `managed.ingress.reconcile` | 300 s           | **Whole-server** ProxySQL desired state only — not per-managed-id. Payload: `serverId`, optional `bindAddress`, `clusters[]` (each cluster: `managedId`, `engine`, `protocolPort` 5432\|3306, writer/reader hostgroups, `backends[]`, `users[]` with passwords resealed for **this** server's daemon key), plus `orgTlsMaterial` scoped to the **server-owner organization** (the org that owns `server.organization_id` — may differ from a grant-placed project's org). Daemon writes compose + full `proxysql.cnf`, materializes TLS under `configDir/proxysql/tls/`, restarts only when static listener section changes, otherwise admin-applies users/servers/rules. Success may reconcile the `managed-ingress` system container when reported. **Never** embeds on `managed.apply`. Offline fail-fast. Failure does **not** flip individual `managed.status` (ingress is server-scoped infrastructure). |
...
leaves `managed.status` untouched. **`managed.ingress.reconcile` failure** also
leaves per-engine `managed.status` untouched. Other command types still write
...
`…/daemon/sync-dev` (all). Dev console: **Sync source to attached checkouts**.
Managed remotes (no checkout) skip source-sync; upgrade them with **Rebuild
daemon and upgrade connected servers** (`POST /api/developer/v1/daemon/update`
after `deno task release:dev` writes the overlay catalog). Co-located is skipped
on both paths.

...
| `POST` | `/api/client/v1/environments/:id/deploy` | session + manage | Merge project+env ComposeDocuments → runtime YAML; create `environment.deploy` on target from persisted `environment.server_id` only (`409 server_placement_required` when unset; body `serverId` ignored). Poll via existing command GET (Postgres only). |
...
| `POST` | `/api/client/v1/environments/:id/stop` | session + manage | Create `environment.stop` on `environment.server_id`; daemon runs `compose down --volumes`, removes hosting site; returns `{ commandId, serverId }`. Poll via command GET. |

...
- **Not built, named so the rows stay honest:** digest / quiet hours on rules, an email verification flow for stranger addresses, folding the legacy `ALERT_WEBHOOK_URL` setting into an instance channel, an admin surface for instance channels, push.

...
import { ALLOWED_PRINCIPAL_SHELLS } from "../../features/principals/principal-options.ts";
import { isCanonicalSshPublicKey } from "../../features/principals/ssh-public-key.ts";
import {
...
} from "../../features/managed/types.ts";
import {
...
} from "../../features/managed/ha-policy.ts";
import type { ServiceOptions } from "../../features/projects/service-options.ts";
...
} from "../../features/fabric/wg.ts";
import { ENVELOPE_PREFIX_DAEMON } from "../../lib/secrets/data-encryption.ts";
 ...
 *
 * `host` renders into `INPUT` (the host's own listeners); `published` into
 * `DOCKER-USER` (a port Docker publishes for a container, matched post-DNAT on
 * the original destination). A published port never traverses `INPUT`.
 */
 ...
 * Must stay in sync with the daemon `server.firewall.reconcile` shape.
 *
 * `sources`: CIDR literals, bare addresses (normalised to `/32` / `/128`) or
 * `any`. On a `published` rule an `accept` from named sources **narrows** the
 * port (those pass, everyone else is dropped for that port); an `accept` from
 * `any` renders nothing, since Docker already accepts. `ports` is one port or
 * an inclusive ascending range; omitted means every port, which only an
 * `drop` / `reject` may say.
 */
export type FirewallCommandRule = {
  id: string;
...
/** Must stay in sync with the daemon `server.firewall.reconcile` shape. */
export type FirewallCommandPolicy = {
 ...
 * Must stay in sync with the daemon `server.firewall.reconcile` shape.
 *
 * `rules` is the **complete** desired set for one server, the way
 * `server.principals.reconcile` carries every account. `controlPlane.tcpPorts`
 * names the co-located control plane's entrypoint (8443, or 80 + 443 under
 * `lets_encrypt`); the daemon treats those as invariants and refuses a
 * default-drop apply on a co-located host that names none. `sshPorts` is the
 * panel's belief; the daemon unions it with `sshd -T`.
 */
export type FirewallReconcileCommandPayload = {
  generation: number;
  mode: FirewallMode;
...
/**
 * Must stay in sync with the daemon `server.firewall.reconcile` shape.
 * `applied: false` with warnings is a refusal the console shows — not a
 * failed command.
 */
export type FirewallReconcileCommandResult = {
  generation: number;
  mode: FirewallMode;
  applied: boolean;
  /** sha256 hex of the rendered rulesets; the drift key. */
  digest: string;
  ruleCount: number;
  ipv6Applied: boolean;
  forwardApplied: boolean;
  /**
   * The effective sshd ports (detected via `sshd -T` ∪ the payload's
   * `sshPorts`, falling back to 22) — each has a host `ACCEPT`. Control-plane
   * ports are guaranteed too but not listed here.
   */
  sshPorts: number[];
  warnings: string[];
  summary: string;
...
const FIREWALL_SCOPES: ReadonlySet<string> = new Set(["host", "published"]);
const FIREWALL_ACTIONS: ReadonlySet<string> = new Set([
  "accept",
  "drop",
  "reject",
]);
const FIREWALL_PROTOS: ReadonlySet<string> = new Set(["tcp", "udp", "any"]);
const FIREWALL_ORIGINS: ReadonlySet<string> = new Set([
  "system",
  "derived",
  "user",
]);
const FIREWALL_MODES: ReadonlySet<string> = new Set([
  "managed",
  "observe",
  "off",
]);
...
/** `8443` or `5432-5440` (inclusive, ascending), normalised. */
...
/** `any`, a CIDR, or a bare address normalised to `/32` / `/128`. */
export function parseFirewallAddress(value: unknown): string | null {
  ...
  if (version === 4) return `${trimmed}/32`;
  if (version === 6) return `${trimmed}/128`;
    ...
    if (parsed === null) {
      throw new Error(
        `${field} entry must be "any", an IP literal or a CIDR: ${
          isString(entry) ? entry : typeof entry
        }`,
      );
    }
...
function requireFirewallEnum(
  ...
  if (!isString(value) || !allowed.has(value)) {
    throw new Error(`rules[${index}].${field} must ${message}`);
  }
  return value;
}
...
function applyFirewallRuleDestinations(
  value: Record<string, unknown>,
  rule: FirewallCommandRule,
  index: number,
): void {
...
function applyFirewallRuleComment(
  value: Record<string, unknown>,
  rule: FirewallCommandRule,
  index: number,
): void {
  if (value.comment === undefined) return;
  if (!isString(value.comment) || !FIREWALL_COMMENT_RE.test(value.comment)) {
    throw new Error(
...
function parseFirewallRule(value: unknown, index: number): FirewallCommandRule {
  ...
  const rule: FirewallCommandRule = {
    id: value.id,
    scope: requireFirewallEnum(
      value.scope,
      index,
      "scope",
      FIREWALL_SCOPES,
      "be host or published",
    ) as FirewallCommandRule["scope"],
    action: requireFirewallEnum(
      value.action,
      index,
      "action",
      FIREWALL_ACTIONS,
      "be accept, drop or reject",
    ) as FirewallCommandRule["action"],
    proto: requireFirewallEnum(
      value.proto,
      index,
      "proto",
      FIREWALL_PROTOS,
      "be tcp, udp or any",
    ) as FirewallCommandRule["proto"],
    sources: parseFirewallAddressList(
      value.sources,
      `rules[${index}].sources`,
      false,
    ),
    origin: requireFirewallEnum(
      value.origin,
      index,
      "origin",
      FIREWALL_ORIGINS,
      "be system, derived or user",
    ) as FirewallCommandRule["origin"],
  };
  applyFirewallRulePorts(value, rule, index);
  applyFirewallRuleDestinations(value, rule, index);
  applyFirewallRuleComment(value, rule, index);
  return rule;
}
...
function parseFirewallCommandPolicy(value: unknown): FirewallCommandPolicy {
  ...
  return {
    inputDefault: value.inputDefault,
    ipv6: value.ipv6,
  };
}

function assertUniqueFirewallRuleIds(rules: readonly FirewallCommandRule[]): void {
  const seen = new Set<string>();
...
/**
 * Parse `server.firewall.reconcile`. Byte-for-byte the daemon's rules: an
 * empty `rules` is legal (the complete set is "invariants only"); a missing
 * `rules` is malformed; ids are unique.
 */
export function parseFirewallReconcilePayload(
  ...
  const generation = requireNonNegativeInt(value.generation, "generation");
  if (!isString(value.mode) || !FIREWALL_MODES.has(value.mode)) {
  ...
  const rules = value.rules.map((rule, index) => parseFirewallRule(rule, index));
  assertUniqueFirewallRuleIds(rules);
  const payload: FirewallReconcileCommandPayload = {
    generation,

...
import type { EnvironmentDeployStorageMaterial } from "./schemas.ts";
import {
...
describe("deploy-validation", () => {
  ...
  it("rejects duplicate catch-all hostings on the same hostname", () => {
    ...
    assertEquals(
      error,
      "multiple catch-all hostings for hostname app.example.com",
    );
  });
  ...
  it("rejects duplicate pathPrefix on the same hostname", () => {
    ...
    assertEquals(
      error,
      "duplicate pathPrefix /api for hostname app.example.com",
    );
  });
  ...
  it("rejects hostname routing when pathPrefix has unsupported characters", () => {
    ...
    assertEquals(
      error,
      "pathPrefix contains unsupported characters for hostname app.example.com",
    );
  });
  ...
  it("rejects a single ACME hosting with forceHttps:false", () => {
    const error = validateDeployHostnameRouting([
      {
        hostingId: "h-acme",
        serviceId: "s1",
    ...
    assertEquals(
      error,
      "forceHttps:false is incompatible with ACME TLS on hostname app.example.com",
    );
  });

  it("rejects a shared-hostname route that disables HTTPS next to an ACME pin", () => {
    const error = validateDeployHostnameRouting([
      {
        hostingId: "h-acme",
        serviceId: "s1",
